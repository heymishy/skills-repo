...
**Phase 5 status (updated 2026-05-14):** Agent behaviour observability remains an open backlog item. No Phase 5 story has been formally scoped for any of the three candidates. The DoR contract diff-check mechanism (part of Phase 4 second-line validation) continues to provide a lightweight precursor to Candidate 3 — PR diff is compared against DoR contract file touchpoints. Full Candidate 3 implementation (plan-level task-to-diff coverage) and Candidate 2 (session replay) remain unscoped as of Phase 5 active delivery. Recommended approach: raise a discovery for this topic in Phase 5 or Phase 6, starting with Candidate 2 (session replay via `parse-session-timing.js`) as a low-effort signal before committing to Candidate 3.
