...
| Overall verdict | **COMPLETE — 8/8 Y implementation (Q1–Q7 closed by p3.2a; Q8 closed by p3.2b).** Q1, Q3, Q4 are trace-backed: `traceHash`, `commitSha`, and `verdict` are present in every `completed` trace entry in `workspace/traces/`. Q2, Q5, Q6, Q7 are **code-only as of Phase 4 close**: the fields (`standardsInjected`, `watermarkResult`, `stalenessFlag`, `sessionIdentity`) exist in the schema and gate code and will be written for regulated stories, but no CI run has ever passed `regulated: true` to `runGate()` — the four fields are absent from all 16 trace files in `workspace/traces/`. Q8 is partially verifiable (`traceHash` present; attestation registry verifiable via `gh attestation verify`). **Adoption of the regulated compliance profile (T3M1 Q2/Q5/Q6/Q7 trace-backed) is gated on running one story with `regulated: true` through the gate and confirming the four fields appear in the resulting trace.** See Section 4 sign-off conditions. |
...
| Conditions (if verdict is "approved with conditions") | All 8/8 T3M1 questions satisfied at implementation level as of Phase 3/4 close. Phase 4 DoD-complete 2026-04-21. Platform cleared for non-dogfood adoption (standard compliance profile) subject to the deployment checklist in `docs/ONBOARDING.md`. Enterprise Bitbucket adopters: implement tamper-evidence Option B (registry-repo) as documented in `docs/HANDOFF.md` Section 2 (Risk R6 note). **Additional condition — regulated compliance profile:** T3M1 Q2/Q5/Q6/Q7 are code-only as of Phase 4 close. `standardsInjected`, `watermarkResult`, `stalenessFlag`, and `sessionIdentity` exist in the gate code and schema but are only written when `regulated: true` is passed to `runGate()` — no CI run has exercised this path. Full 8/8 trace-backed T3M1 requires one regulated story run through the gate with `regulated: true` and the four fields confirmed present in the resulting trace file. **Adoption of the regulated compliance profile is gated on this.** Teams using the standard compliance profile are unaffected. |
