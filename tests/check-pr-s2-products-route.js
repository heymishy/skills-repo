    ...
    try {
      ...
      var mockPoolNeverSynced = {
        query: async function(sql) {
          ...
          if (/SELECT dod_status_counts, health_counts, test_coverage, synced_at FROM product_rollups/i.test(sql)) return { rows: [] };
          return { rows: [] };
        }
      };
      ...
      var mockPoolSynced = {
        query: async function(sql) {
          if (/SELECT name, tenant_id FROM products/i.test(sql)) return { rows: [{ name: 'Acme', tenant_id: 't1' }] };
          if (/SELECT dod_status_counts, health_counts, test_coverage, synced_at FROM product_rollups/i.test(sql)) return { rows: [{ dod_status_counts: '{"complete":1}', health_counts: '{"green":0,"amber":0,"red":0,"unknown":0}', test_coverage: '{}', synced_at: syncedAt }] };
          return { rows: [] };
        }
      };
      var htmlSynced = null;
      ...
      var mockPool = {
        query: async function(sql) {
          ...
          if (/SELECT dod_status_counts, health_counts, test_coverage, synced_at FROM product_rollups/i.test(sql)) {
            return { rows: [{ dod_status_counts: '{"complete":1}', health_counts: healthCountsJson, test_coverage: '{}', synced_at: syncedAt }] };
          }
          return { rows: [] };
        }
      };
