...
function reset() {
  ...
  if (typeof tenantPlan.resetPlanState === 'function') tenantPlan.resetPlanState();
}
...
(function() {
  ...
  var result = tenantPlan.checkJourneyCap('tenant-a', 3);
...
(function() {
  reset();
  tenantPlan.setCapReader(function() { return 3; });
  var result = tenantPlan.checkJourneyCap('tenant-a', 2);
...
(function() {
  ...
  var result = tenantPlan.checkJourneyCap('tenant-a', 1);
...
(function() {
  ...
  var before = tenantPlan.checkJourneyCap('tenant-paid', 5);
  ok('trial (default) plan state is still capped', before.allowed === false);

  tenantPlan.setPlanState('tenant-paid', 'paid', 'active');
  var after = tenantPlan.checkJourneyCap('tenant-paid', 5);
...
(function() {
  ...
  tenantPlan.setPlanState('tenant-b', 'paid', 'active');
  var whilePaid = tenantPlan.checkJourneyCap('tenant-b', 10);
  ok('while paid+active — unlimited', whilePaid.allowed === true);

  tenantPlan.setPlanState('tenant-b', 'trial', 'canceled');
  var afterCancel = tenantPlan.checkJourneyCap('tenant-b', 10);
...
(function() {
  reset();
  process.env.MAX_JOURNEYS_PER_TENANT = '1';
  tenantPlan.setPlanState('tenant-c', 'paid', 'past_due');
  var result = tenantPlan.checkJourneyCap('tenant-c', 1);
...
(function() {
  reset();
  process.env.MAX_JOURNEYS_PER_TENANT = '1';
  tenantPlan.setPlanState('tenant-d', 'paid', 'active');
  tenantPlan.resetPlanState();
  var result = tenantPlan.checkJourneyCap('tenant-d', 1);
...
(function() {
  reset();
  var result = tenantPlan.checkJourneyCap('tenant-e', 999);
