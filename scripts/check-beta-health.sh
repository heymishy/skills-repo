...
BILLING_ERRORS=$(timeout 10 fly logs --app "$APP" 2>/dev/null | grep "Journey limit reached" || true)
...
PG_ERRORS=$(timeout 10 fly logs --app "$APP" 2>/dev/null | grep "\[journey-store\] PG write error" || true)
if [ -n "$PG_ERRORS" ]; then
...
REDIS_ERRORS=$(timeout 10 fly logs --app "$APP" 2>/dev/null | grep "\[session\] Redis write error" || true)
