...
| /test-plan (PCI/compliance-classified stories) | claude-sonnet-4-6 | EXP-007: Haiku D3=0.7 on T5 both trials — confirmed systematic scope-mixing in NFR-SEC-1 (adds positive AC1 gateway assertion alongside negative log constraints). Sonnet D3=1.0 both trials. Gap addressable via /test-plan SKILL.md NFR scope rule — pending update. | true (`experiment_id: EXP-007-testplan-rubric`, 2026-05-16) | Downgrade to Haiku once NFR scope rule added to SKILL.md and re-validated |
