...
- **Story S1.2 (Primary-to-standby database replication)** describes connecting the standby database to the CDE-scoped primary infrastructure — an explicit CDE expansion. Its Architecture Constraints section lists C1 and C3 only. C2 (PCI DSS QSA gate) is absent despite this being a CDE-expanding architectural change that requires QSA assessment per discovery C2 and ADR-019.

- **Story S2.2 (Automated failover activation)** describes promoting the standby to live payment processor (AC3: "the standby payment processing service is set to `mode: active`...") — another explicit CDE expansion. Its Architecture Constraints section lists C3 only, with no C2 reference.
