...
# Application: Trans-Tasman Payment Routing Service (Westpac NZ)
...
**Owner:** Payments Technology, Westpac NZ
...
**Description:**
New intra-group payment routing service that routes NZ retail customer payment instructions to Australian beneficiary accounts via a proprietary Westpac group channel, bypassing the standard SWIFT international payment flow. Positions between Westpac NZ and Westpac AU are settled in the group treasury books at end-of-day. Customer-facing settlement time: 2 hours. Target pricing: under $5 for payments up to $10,000 (standard SWIFT route applies above $10,000).

**Hosting:** Azure (shared with existing digital banking platform)
**Technology stack:** Node.js routing service, Azure Service Bus (payment instruction queue), integration with Westpac NZ AML/CFT screening service, integration with Westpac Group Treasury API (position management)
...
| Interface ID | Application | Interface type | Data transferred | Notes |
|-------------|-------------|---------------|-----------------|-------|
| TTPS-SET-001 | Westpac Group Treasury — Net Settlement | Internal — end-of-day batch | NZD/AUD net positions from all intra-group transactions that day | RBNZ FX transaction reporting obligation — see TTPS-RISK-003 |
...
| TTPS-REG-002 | AUSTRAC Correspondent Banking — Westpac AU | External (via Westpac AU compliance) | Originator information bundle: name, NZ account number, NZ address, amount, purpose | Required for Westpac AU to satisfy AUSTRAC correspondent banking obligations; format and content requirements must be confirmed with Westpac AU compliance team |
...
|-----------|-----------|---------|--------|
| RBNZ AML/CFT Act 2009 — sanctions screening on all outbound international payments | RBNZ | NZ | Applies regardless of routing channel; Westpac NZ retains primary AML/CFT liability |
...
| AUSTRAC — correspondent banking rules — originator information requirements | AUSTRAC | AU | Westpac NZ must provide complete originator information with each credit instruction to Westpac AU to satisfy AUSTRAC's correspondent banking obligations on the Australian leg |
...
Westpac NZ's SWIFT correspondent relationship with JPMorgan Chase (for international AUD payments) is governed by a bilateral correspondent banking agreement. The agreement includes a standard correspondent bank clause requiring Westpac NZ to provide written notification to JPMorgan Chase before:
...
**Severity:** BLOCKER — correspondent bank agreement breach exposes Westpac NZ to credit relationship risk with JPMorgan Chase. The correspondent relationship supports not only retail NZD/AUD payments but also Westpac NZ's trade finance and wholesale banking transactions. Breach may trigger review of the broader correspondent relationship.
...
| TTPS-RISK-004 | AUSTRAC originator information format — the required format and fields for Westpac AU to satisfy its AUSTRAC correspondent banking obligations on the AU leg have not been confirmed with Westpac AU compliance | MEDIUM |
