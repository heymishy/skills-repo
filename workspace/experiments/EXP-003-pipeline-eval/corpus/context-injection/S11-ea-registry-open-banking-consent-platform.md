...
# Application: Open Banking Consent Platform (Westpac NZ)
...
**Owner:** Digital Open Banking Programme, Westpac NZ
...
### Data source interfaces (authorised sharing pathways — READ ONLY from Westpac systems)
...
| OBCP-SRC-003 | Westpac Analytics Engine | Internal API — read only | Enriched insights: spending category breakdowns, estimated monthly income, 90-day projected balance | **Derived data — separate consent scope assessment required before enabling** |
...
| NZ Open Banking Framework — revocation and deletion | DIA | On consent revocation, third party must delete all data within 24 hours; Westpac must be able to demonstrate deletion compliance on request |
...
**OBCP-SRC-003 (Westpac Analytics Engine — enriched insights) requires separate legal assessment before enablement.**

Enriched insights (spending categories, income estimation, projected balance) are not raw data from Westpac's core banking system. They are outputs of Westpac's proprietary internal analytics models applied to transaction data. These outputs constitute inferences about the customer — a distinct category of personal information under the Privacy Act 2020.

Whether a customer consent for "transaction data" covers sharing Westpac's derived analytical inferences with third parties is a consent scope question that requires Privacy Act legal advice. The concern:
- The customer is consenting to share their transaction history
- Westpac is using that history as input to its own models
- The model outputs (enriched insights) are Westpac's internal analytical products
...
- Westpac Analytics Engine (enriched insights — gated by Privacy Act assessment)
