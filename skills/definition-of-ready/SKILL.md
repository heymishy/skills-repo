...
Read the file list from the story's DoR contract's own `Estimated touch points` → `Files:` field (the same field H1's Contract Proposal already requires every story to fill in) — this is the deterministic source for "the story's declared touched files," matching H-INF/H-MIG's own precision in naming an exact field to read (`infraPlanPath`, `migrationReviewPath`) rather than leaving the source implicit. Run `scripts/check-design-tokens.js` against each file in that list: `node scripts/check-design-tokens.js <file> [--design-md <path>]`. The script extracts the token palette from `DESIGN.md`'s `## Color tokens` section (via `extractDesignTokens`) and scans the target file for hardcoded hex color values not present in that token set (via `scanFileForNonTokenColors`). Evaluate the following cases:
