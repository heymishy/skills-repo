...
- **pr.1 — /prioritise SKILL.md: candidate intake and framework selection (2026-04-28):** Creates `.github/skills/prioritise/SKILL.md` (partial — opening, candidate intake, and framework selection sections). Introduces the three prioritisation frameworks (WSJF/cost-of-delay, RICE/Reach-Impact-Confidence-Effort, MoSCoW/Must-have-Should-have-Could-have-Won't-have) with plain-language descriptions. Instructs the skill to acknowledge candidate items, ask at most two clarifying questions before suggesting a framework, state a rationale for the suggestion, wait for an explicit confirm or override, and accept overrides without re-arguing. `tests/check-pr.1.js` (12 assertions). Artefact chain at `artefacts/2026-04-27-prioritise-skill/`.
