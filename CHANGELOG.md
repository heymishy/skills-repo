...
- **`/discovery` and `/workflow` SKILL.md — `track` field added to state update instructions (2026-04-18):** Both skills now include the `track` field (with valid enum values: `short`, `standard`, `programme-workstream`, `library`) when creating new `features[]` entries in pipeline-state.json. Previously neither skill specified `track` despite it being `required` in the schema, causing agents to guess invalid values (D12 learning). `/discovery` defaults to `"standard"` unless the operator explicitly routes via short-track or programme. `/workflow` reconciliation infers `track` from artefact shape.
