...
- **fix: validate-trace.ps1 compatible with PowerShell 7 strict mode (2026-05-08):** Added `Get-JsonProp` helper function and replaced all direct `$feature.stories`, `$feature.epics`, `$feature.health`, `$feature.blocker`, `$feature.slug`, and equivalent story/epic property accesses in `Check-TestPlanCoverage` and `Check-UnresolvedBlockers` with safe PSObject property lookups. `Set-StrictMode -Version Latest` on PowerShell 7 (Linux CI) throws `PropertyNotFoundException` when accessing a missing property on a parsed JSON object; several features in pipeline-state.json legitimately omit `stories` or `epics`. Also added `Read-TraceConfigList` and `Read-FeatureTracks` helpers and updated `Check-DiscoveryExists` to respect `reference_dirs` and `tracks_without_discovery` from `.github/trace-validation.yml` — matching the behaviour of the `.sh` equivalent. Fixes `ps1-exits-0-on-valid-repo-with-ci-flag` CI test for all active branches.
