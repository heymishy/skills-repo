...
**Follow-up actions:**
1. ✅ **Done 2026-08-09.** Live-browser recheck post-deploy (`new-feature-f2aaa734`, the same `bri-s3.1` mock-fixture content, on the `Women's mentorship` product): opened its Discovery resume link before/after — kanban card stayed at "7 artefacts / definition-of-ready / Not ready to advance" with zero change, matching the automated suite. Caveat: "Resume conversation" now routes through a newer read-only "view completed stage" viewer (`/journey/:id/stage/:stage`) rather than landing directly on the old auto-fire chat page, so this specific click didn't exercise the exact previously-vulnerable code path byte-for-byte — the 8 passing unit tests against the real handler functions remain the primary proof; this live check is corroborating. See `workspace/capture-log.md`, 2026-08-09.
