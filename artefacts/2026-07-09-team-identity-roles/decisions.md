...
---
**2026-07-14 | ARCH | tir-s9, fix-forward story filed**
**Decision:** Filed tir-s9 ("The identityKey passed to login-time role resolution must be each person's own identity, not the shared tenantId") after independently confirming that tir-s7's fix (PR #467, merged), while correct in its own query logic (`resolveRoleForPerson(pool, identityKey, tenantId)`), is fed the wrong `identityKey` in production. `server.js`'s wiring of `setGetRoleForTenant` collapses both arguments into the same value: `resolveRoleForPerson(_userRolesPool, tenantId, tenantId)`. Every login call site (`routes/auth.js`'s GitHub and Google callbacks) calls `getRoleForTenant(req.session.tenantId)` with a single argument, so `identityKey` always equals `tenantId`. For a solo tenant or email/password login this is harmless (`tenantId` already equals that one person's own identity), but once `TENANT_ORG_ALLOWLIST` is configured — the shared-tenant case this entire epic exists to support — `resolveTenant()` returns the *same* org-level `tenantId` for every teammate on that org, so every teammate's login calls `resolveRoleForPerson` with the shared org name as `identityKey`. `resolvePersonForIdentity` can only resolve one specific person from that shared string, so every teammate on a shared GitHub-org tenant login reproduces tir-s7's original bug (an arbitrary teammate's role is returned, not the logging-in person's own role) one layer removed — the query logic tir-s7 fixed never gets exercised with the right input in production. Confirmed independently by direct code reading (`server.js` lines ~282-284, `routes/auth.js` lines ~201 and ~302) before writing any test, then confirmed further by writing `tests/check-tir-s9-per-person-identitykey-login-fix.js` and running it against the pre-fix code: T1 (person Y resolves engineer), T5 (adapter forwards identityKey), and T6 (two identities sharing one tenantId resolve to two different roles) all failed against unfixed code — T6 specifically returned `admin` when `engineer` was expected for person Y, the exact collision this decision describes — while T2/T3a/T3b/T4 (regression cases for admin-row-order, solo tenant, email, and Google) passed unmodified, confirming the fix's blast radius is precisely the shared-tenant case and nothing else.
**Alternatives considered:** (1) Treat this as covered by tir-s7's existing fix and not file a new story, on the theory that `resolveRoleForPerson`'s logic is correct — rejected because "the query logic is correct" and "the bug is fixed in production" are different claims, and only the second one matters to Metric 1; tir-s7's own tests called the corrected function directly with hand-picked distinct identity strings, which cannot detect a caller-side argument-collapsing bug like this one. (2) Silently patch `server.js`'s wiring inline without a new story artefact — rejected per this repo's established fix-forward convention (tir-s7, tir-s8) and CLAUDE.md's Artefact-first rule; a behavioural change to already-shipped, security-relevant login logic requires its own story → test-plan → review → DoR chain, not a silent patch.
**Rationale:** This is a real, security-relevant regression — a teammate on a shared tenant can resolve into a *different* teammate's role, in either direction (an engineer resolving into an admin's role, or vice versa, depending on row/insertion order) — on the exact multi-person, shared-tenant path (`TENANT_ORG_ALLOWLIST`) this whole epic was built to support. Per the operator's explicit direction, and because the product is pre-launch with zero live paying customers (`product/roadmap.md`), this finding is described accurately and in full technical detail in the story, its tests, its PR, and its commit messages — no euphemism or redaction.
**Related-but-deferred finding (not fixed by tir-s9):** while investigating, confirmed that a Google-authenticated person manually added as a teammate to a GitHub-org-shared tenant (via `team-management.js`'s `addOrUpdateTeammate`, whose form label explicitly accepts "Google email") can never see their assigned role on subsequent logins — their own Google login always recomputes `req.session.tenantId = userInfo.sub` (their personal Google identity), which never matches the shared org `tenant_id` their `team_memberships` row was actually written under, so they silently default to the `user` role every time. This is a different bug shape (silent role loss, not role collision) than tir-s9 fixes and is explicitly Out of Scope for this story — flagged here as a candidate follow-up story, not silently dropped. Revisit if/when Google-authenticated teammates on shared GitHub-org tenants become a real usage pattern.
**Made by:** Hamish King (Founder/Operator) + Claude (agent), 2026-07-14.
**Revisit trigger:** None for the fixed bug — being fixed immediately as tir-s9, not deferred. The related Google/shared-tenant finding above should be revisited as its own story if/when that usage pattern becomes real.
---
