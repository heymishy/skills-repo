## Definition of Ready: An admin adds a teammate by identity and assigns a role
...
Full detail (including H8-ext, H-NFR/2/3, H-NFR-profile, H-GOV, H-ADAPTER, H-INF, H-MIG): see `tir-s3-dor-contract.md`.
...
Proceed: Yes
Story: An admin adds a teammate by identity and assigns a role — artefacts/2026-07-09-team-identity-roles/stories/tir-s3.md
...
- Depends on tir-s1's people/team_memberships schema already existing —
  do not re-implement or duplicate that schema.
- AC5 requires rejecting an add for an identity with no existing `people`
  row — do not build a placeholder-record mechanism; this is explicitly
  deferred (see Out of Scope).
- Tenant-scoped authorization (ADR-025): an admin of tenant A must never
  be able to add/assign roles for tenant B.
