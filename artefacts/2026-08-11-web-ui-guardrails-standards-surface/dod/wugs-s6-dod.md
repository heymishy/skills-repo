...
**Deviation:** The DoR's own REQUIRED pre-merge step — a real manual test against a disposable sandbox GitHub repo confirming the branch-ref/Contents/Pulls API response shapes match what the tests mock (CLAUDE.md's mock-shape-verification rule, `tir-s5` precedent) — was not performed before PR #726 merged. The PR description's checklist item for this step remained unchecked and no outcome was ever recorded. This is a process deviation, not an AC deviation: all 6 ACs and both NFRs are fully covered by automated tests exercising a hand-authored mock of the GitHub API, but `realCreateGuardrailPr` has never actually been exercised against the real GitHub API. Logged as a RISK-ACCEPT in `decisions.md` (2026-08-13 entry) immediately on discovery, once the operator confirmed the step had not been run. **Resolved 2026-08-14** — the manual sandbox verification was subsequently run and the RISK-ACCEPT formally closed (see Follow-up action #1 and DoD Observation #4). The deviation record above is kept as an accurate historical account of what happened at merge time, not retroactively erased.

**Follow-up actions:**
1. ~~Run the manual sandbox verification~~ — done, 2026-08-14 (see DoD Observation #4). Real branch + PR opened against a disposable sandbox repo (`heymishy/skills-repo-sandbox`) via `realCreateGuardrailPr`'s actual GitHub API calls, independently confirmed via `gh pr view`. `decisions.md`'s RISK-ACCEPT entry (2026-08-13) was formally closed by a RISK-ACCEPT-RESOLVED entry (2026-08-14).
2. No real API shape mismatch was found — this action does not apply.
...
4. **Backfilled 2026-09-11, following a repo-wide DoD-verification-method stocktake.** The RISK-ACCEPTed manual sandbox verification this DoD's own Follow-up action #1 flagged as outstanding had, in fact, already been run and closed the very next day (2026-08-14) — `decisions.md`'s own RISK-ACCEPT-RESOLVED entry records the operator running `wugs-s6-manual-verification.js` against a disposable sandbox repo (`heymishy/skills-repo-sandbox`), producing a real branch + PR via `realCreateGuardrailPr`'s actual GitHub API calls, independently confirmed via `gh pr view 1 --repo heymishy/skills-repo-sandbox` (state: OPEN). This DoD artefact itself was simply never updated to reflect that closure — the same stale-DoD-text-vs-already-resolved-reality pattern found and fixed this session for `sob-s1`/`sob-s2`/`sob-s3`. Per this session's own "check `decisions.md` first" lesson (captured in `workspace/capture-log.md`), no fresh Chrome or sandbox verification was needed here — the existing record was sufficient to close the gap. No `pipeline-state.json` staleness found for this story (its `dodStatus`/`stage` were already correctly recorded).
...
4. Is the manual sandbox-verification step's resolution (Outcome section, closed 2026-08-14) clearly evidenced, not just asserted?
