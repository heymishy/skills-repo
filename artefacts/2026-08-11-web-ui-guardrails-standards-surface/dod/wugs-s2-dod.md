...
| Performance — reasonable fetch timeout expected | ✅ (closed 2026-08-14, see DoD Observation #4) | No `AbortController`/timeout wrapper existed in `wugs-s1`'s `fetchRepoPath`/`realFetchRepoPath` adapter, which this story consumes. Found during final story-level review; RISK-ACCEPTed in `decisions.md` (2026-08-12) and logged in `nfr-profile.md`'s Gaps table, since the fix belonged in the already-merged shared adapter, not in this story's own touchpoints. **Closed by `wugs-s14`** (PR #736, merged 2026-08-13, own DoD dated 2026-08-14): both `fetchArtefact` and `realFetchRepoPath` now inherit a 10s `AbortController`-based timeout identically, with 5 passing tests confirming the hang/timeout, unchanged-fast-path, and timer-cleanup behaviour. |
...
The deviation record below is kept as an accurate historical account of what shipped at merge time (the RISK-ACCEPTed timeout gap), even though it was closed 2 days later — not retroactively erased.

**Follow-up actions:**
1. ~~Add `AbortController`-based timeout handling to `wugs-s1`'s `fetchRepoPath`/`realFetchRepoPath` adapter~~ — done, `wugs-s14` (PR #736, merged 2026-08-13). See DoD Observation #4.
...
4. **Backfilled 2026-09-11, following a repo-wide DoD-verification-method stocktake.** The RISK-ACCEPTed fetch-timeout gap this DoD's own Follow-up action #1 flagged as outstanding had, in fact, already been closed a day after the RISK-ACCEPT was logged — `wugs-s14` (PR #736, merged 2026-08-13, own DoD dated 2026-08-14) is a dedicated story that adds exactly the `AbortController`-based timeout this gap called for, to the same shared adapter (`fetchRepoPath`/`realFetchRepoPath`) named in the RISK-ACCEPT's revisit trigger. This DoD artefact was simply never updated to reflect that closure — the same stale-DoD-text-vs-already-resolved-reality pattern found and fixed this session for `sob-s1`/`sob-s2`/`sob-s3`/`wugs-s6`. Per this session's "check `decisions.md`/existing DoDs first" lesson, no fresh Chrome or code work was needed here — `wugs-s14`'s own DoD already contained complete, passing evidence. `pipeline-state.json`'s `m1` metric evidence already correctly references `wugs-s14`'s closure and lists it in `contributingStories`; no staleness found there.
