# Definition of Ready: Render aggregate AC coverage on the product rollup view (pr-s6)

**Story reference:** artefacts/2026-07-16-product-rollup/stories/pr-s6.md
**Test plan reference:** artefacts/2026-07-16-product-rollup/test-plans/pr-s6-test-plan.md
**Contract:** artefacts/2026-07-16-product-rollup/dor/pr-s6-dor-contract.md
...
| H4 | Out-of-scope section is populated | ✅ | Trend over time, per-AC detail both correctly deferred |
...
| H9 | Architecture Constraints field populated; no Category E HIGH findings | ✅ | Shared calc-method [ASSUMPTION] with pr-s5, ADR-018 cited; Category E score 3/5 in review, no HIGH |
...
| W3 | MEDIUM review findings acknowledged | ✅ | — | N/A — findings 6-M1, 6-L1 were fixed, not just acknowledged |
...
| W5 | No UNCERTAIN items in test plan gap table | ✅ | — | Test plan flags the shared calc-method [ASSUMPTION] explicitly with a stated mitigation, matching pr-s5 |
...
Note: same calc-method [ASSUMPTION] caveat as pr-s5 applies — implement exactly the blended method as currently specified, keep consistent with pr-s5's own implementation.
...
Proceed: Yes
Story: Render aggregate AC coverage on the product rollup view — artefacts/2026-07-16-product-rollup/stories/pr-s6.md
...
Constraints:
- Compute aggregate AC coverage as sum-of-verified over sum-of-total across
  features (blended), using the SAME method as pr-s5's test coverage
  calculation for consistency — do not implement a different method here.
- Features with no acTotal/acVerified data must be excluded from both
  numerator and denominator — never treated as 0%.
- AC coverage and test coverage must be clearly, distinctly labelled on the
  same page — not ambiguous unlabelled numbers next to each other.
- Do not build trend-over-time or per-AC detail — both explicitly out of
