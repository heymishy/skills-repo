...
4. **Rollup views computed, cached, and rendered in the web UI**, extending the existing `/products/:id` page. Views cover at minimum:
   - **Overall discovery scope** across the product's features, and **epic/feature taxonomy grouping**.
   - **Aggregate health** — a count of features by health status (green/amber/red/unknown), shown with the same status vocabulary and labels already established in this codebase (`.github/scripts/viz-functions.js`'s `fleetHealthLabel`: ✓ Healthy / ⚠ Warning / ✕ Blocked / ? Unknown), plus a single derived overall product-health signal using the same red-takes-precedence rule already applied per-feature elsewhere in this codebase (`viz-functions.js`'s `featureActionMeta`: any feature red → product shows red; else any amber → amber; else green). Fetched directly from each feature's `health` field (already named as a fetched field in item 3) — this closes a real gap in the problem this discovery names: an operator's stated need to see "aggregate delivery health" (see Who It Affects) was not actually represented as a rollup dimension in an earlier pass of this scope.
   - **Aggregate test coverage** — sum of `testPlan.passing` / sum of `testPlan.totalTests` across features (a blended percentage — see Assumptions and Risks), with per-feature detail available, not just a single blended number.
   - **Aggregate acceptance-criteria coverage** — sum of `acVerified` / sum of `acTotal` across features, using the same blended-percentage approach as test coverage for consistency. `acTotal`/`acVerified` were already named as fetched fields in item 3 but, like health, had not been carried through into an actual rollup dimension in an earlier pass — corrected here for the same reason.
   - **Aggregate DoD status** — count of features at each stage, with `dodStatus` breakdown.
...
[ASSUMPTION] Aggregate test coverage and aggregate AC coverage should each be computed as a sum-of-passing(or verified)-over-sum-of-total across referenced features (a blended percentage), rather than an average-of-per-feature-percentages — unconfirmed, requires /clarify before scope is locked. These two methods produce materially different numbers when features have very different total test/AC counts, and the choice affects what the rollup number actually communicates. The two metrics (test coverage, AC coverage) should use the same method as each other for consistency, whichever is chosen.

[ASSUMPTION] The red-takes-precedence rule for deriving an overall product-health signal (any feature red → product red; else any amber → amber; else green) is the right rule to reuse from `featureActionMeta`'s existing per-feature logic — unconfirmed, requires /clarify before scope is locked. An alternative (e.g. a weighted or percentage-based health score) might better reflect a large product where a single red feature among 49 shouldn't necessarily read the same as a single red feature among 3.
...
This discovery contains 3 unconfirmed assumptions that affect scope and benefit measurement. Before proceeding further, consider running `/clarify` to resolve:

- Aggregate test coverage and aggregate AC coverage should each be computed as a sum-of-passing(or verified)-over-sum-of-total across referenced features (a blended percentage), rather than an average-of-per-feature-percentages.
- The red-takes-precedence rule for deriving an overall product-health signal is the right rule, versus a weighted or percentage-based alternative.
...
None of these assumptions block `/benefit-metric` — all affect implementation detail rather than the metrics themselves — but all should be resolved before `/definition` locks story scope.
