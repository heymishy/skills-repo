...
- **`src/web-ui/routes/skills.js`** — add `csrfGuard(req, res)` to the 2 form-path handlers (`handlePostSkillSessionHtml`, `handlePostCommitHtml`) — the JSON-path siblings are untouched, per the story's own scope boundary. (`annotation.js`/`artefact-view.js` are no longer touched — see 2026-08-24 scope correction below.)
...
- **4 new test files** (`tests/check-rcfc-s1-journey-forms-csrf.js`, `tests/check-rcfc-s1-skills-sessions-csrf.js`, `tests/check-rcfc-s1-products-csrf.js`, `tests/check-rcfc-s1-legacy-login-csrf.js`), 23 tests total, matching `sec-perf-s3`'s established full-router-dispatch integration-test convention.
...
- `POST /api/artefacts/:slug/:file/annotations` — removed 2026-08-24 after investigation found it is a JSON/fetch-only route with no live server-rendered `<form>` target (`annotation.js`, `artefact-view.js` untouched). See `decisions.md`.
...
| AC2 — skill-session-form + skill-commit-form (2 routes) | Same dispatch pattern (annotations route removed 2026-08-24 — JSON/fetch-only, out of scope, see decisions.md) | Integration |
...
**Files:** `src/web-ui/routes/journey.js`, `src/web-ui/routes/skills.js`, `src/web-ui/views/commit-view.js`, `src/web-ui/routes/products.js`, `src/web-ui/utils/html-shell.js`, plus 4 new test files under `tests/`.
**Services:** None.
...
- AC2 ↔ 4 tests: reject/round-trip pairs for skill-session-form and skill-commit-form — ✅ aligned. (Annotations removed 2026-08-24 — JSON/fetch-only, no live form target — consistently reflected in the story, decisions.md, this contract, and the test plan.)
...
| H3 | Every AC has at least one test in the test plan | ✅ | AC1: 12, AC2: 4, AC3: 4, AC4: 3, AC5: folded into the above (23 total) |
