...
An admin enters a teammate's email address and a role (role selection is required, no default); the platform sends that person a unique, per-person invite email, valid for 24 hours, **reusing the existing `sendInvitationEmail`/Resend adapter already built and production-wired for `2026-07-30-agency-client-organisations`'s own invitation flow** (corrected at /definition: an earlier claim in this artefact that wuce "has never sent an email" was wrong — a real, D37-compliant email adapter already exists; see `decisions.md`). The invitee clicks the link within that window and authenticates via the existing shared magic-link strategy (extended to a third invitation type, alongside the two it already dispatches), joining the tenant directly with the role the admin assigned — without the admin having to separately "add by identity" afterward. If the tenant is at its member-count cap (a basic cap keyed to the tenant's existing plan tier — see the corrected Constraints section below, this is new prerequisite scope built as part of this epic, not existing Stripe per-seat billing) at the moment the invitee tries to accept, the join is blocked (checked at acceptance, not at invite creation). This removes both the "admin must act first, one person at a time" bottleneck and the "admin must manually share a link some other way" step, without requiring a full invite-management UI (revoke, resend, audit trail) in the first cut.
...
- **Corrected at /definition:** no seat/plan-quantity limit exists anywhere in wuce today — billing (`billing.js`) tracks a plan TIER per tenant (starter/pro), never a seat COUNT, and `product/roadmap.md` explicitly lists "per-seat/usage-based Stripe billing" as deferred, never built. A basic per-tenant member-count cap (keyed to the tenant's existing plan tier, not full Stripe per-seat billing) is now built as part of this epic's own MVP scope — see `wsi-s4` and `decisions.md`'s correction entry.
