...
---
**[2026-08-15] | CORRECTION | inner coding loop, wsi-s1 Task 2 (post-review, pre-Task-3)**
**Decision:** Task 2's initial implementation (commit `a32a32c9`) hand-built the invite link as a plaintext URL around the raw `team_invitations.team_invitation_id` (`/invite/redeem?teamInvitationId=<id>`) and passed it directly to `invitation-email.js`'s `sendInvitationEmail`. This does not satisfy AC2's exact wording ("a link containing the signed invite token") or the signed-off DoR contract's exact wording ("a link containing a signed JWT carrying `teamInvitationId`") — both require a signed JWT, not a raw, guessable, unsigned database primary key used as the sole redemption credential. Corrected: `handleCreateInvite` must call `auth/magic-link-strategy.js`'s existing `issueMagicLink(email, { teamInvitationId })`, exactly mirroring `routes/agency-provisioning.js`'s own `handlePostInviteUser` (the direct precedent for this same "invite a person by email, send a signed magic link" shape), including its documented `!sent.success` check (`issueMagicLink`'s underlying `passport-magic-login` `.send()` swallows a rejected `sendMagicLink` into `{success:false}` rather than rejecting the promise, so an adapter failure must be checked explicitly or it is silently reported as success). This was caught by two independent post-Task-2 review subagents (spec-compliance and code-quality) run per this session's established two-stage review discipline, then independently verified against the real story/DoR text and the real sibling precedent code before acting.
**Alternatives considered:** (1) Keep the plaintext-ID link and treat the AC2/DoR wording as aspirational, RISK-ACCEPTing the gap — rejected, this is a genuine security weakening (an unsigned, non-cryptographically-random ID as a bearer token) with a real, already-built, ADR-026-reuse-compliant alternative directly available; not a style nitpick to defer. (2) Build a new, separate signing mechanism scoped to team invites only — rejected, would duplicate `magic-link-strategy.js`'s existing JWT signing/verification machinery that `wsi-s2` (invite redemption) is already planned to extend via the same `payload.teamInvitationId` dispatch key, per this feature's own /definition research.
**Rationale:** The plan document (`artefacts/2026-08-14-wuce-self-serve-invites/plans/wsi-s1-plan.md`) itself specified the flawed plaintext-link code in Task 2's Step 3 — this is an authoring gap in the implementation plan, not a subagent deviation from a correct plan (the subagent implemented exactly what was specified). Root cause: the plan was written from the story/DoR text's high-level shape without cross-checking the exact sibling precedent's mechanism (`issueMagicLink`, not a direct `sendInvitationEmail` call) at plan-writing time. Folding in the `!sent.success` check at Task 2 (rather than deferring all failure handling to Task 4, as originally planned) is necessary because `issueMagicLink`'s failure-swallowing behaviour means omitting the check would silently report a failed send as a successful invite creation — a worse outcome than the original plan's uncaught-throw behaviour. Task 4 (AC5) is expected to need no further core code change as a result — see the plan's own Task 4 correction note.
**Made by:** Claude (agent) — found via two dispatched review subagents (spec-compliance, code-quality), independently verified against `stories/wsi-s1-admin-creates-invite.md` AC2, `dor/wsi-s1-dor-contract.md`, `routes/agency-provisioning.js`, and `auth/magic-link-strategy.js` before acting
**Revisit trigger:** None — this is now the corrected, locked-in mechanism for wsi-s1's invite-email link, and the precedent `wsi-s2` should follow for its own redemption-side dispatch.
---

---
**[2026-08-15] | CORRECTION | wsi-s1 final story-level check (before /verify-completion)**
**Decision:** The branch-setup baseline record the plan document cites ("33 pre-existing failures... see decisions.md's 2026-08-15 branch-setup RISK-ACCEPT entry") does not exist on this branch. It was written to `decisions.md` in commit `6a290867` on `master`, but `feature/wsi-s1` had already branched off one commit earlier (`38a9fe84`), so that commit is a sibling, not an ancestor, and its `decisions.md` change never reached this branch — a real, verified instance of this repo's own documented "artefact lands on the wrong checkout" hazard (`workspace/learnings.md`, 2026-08-12 entry), here affecting a commit rather than a live Edit call. This entry is the actual record for this branch.
**Ground truth (independently re-established, not assumed):** Checked out the exact `6a290867` commit into a disposable worktree and ran the full suite there: `516 file(s) run, 33 failed`. Ran the full suite on `feature/wsi-s1` before any fix: `517 file(s) run, 36 failed` (517 = 516 + wsi-s1's own new test file). Diffed the two failure lists exactly: 3 new failures — `check-d2-banner-exit-permission-visibility.js`, `check-d4-nfr-security-review-and-hardening.js` (both a real, expected consequence of `wsi-s1` adding a 13th `requireAdmin`-gated route — both files hardcode an exhaustive count specifically so a new admin-gated route can't be added without their own enumeration being re-run, per their own failure messages; fixed by updating both counts 12→13 and adding `/api/team/invites` to D4's `expectedRoutes` list and D2's block-level assertions, commit `f0b03656`), and `check-spc5-capture-completeness-script.js` (passed standalone 3/3 times, unrelated to any file `wsi-s1` touches; treated as a full-suite-context flake, consistent with `workspace/learnings.md`'s prior finding on full-suite run reliability). Re-ran the full suite on `feature/wsi-s1` after the D2/D4 fix: `517 file(s) run, 33 failed` — failure list diffed byte-for-byte identical to the true baseline's 33, confirming zero net regressions and that `spc5` was indeed a flake, not a real gap.
**Alternatives considered:** (1) Trust the plan document's citation at face value and RISK-ACCEPT all 3 "new" failures as unverified pre-existing noise — rejected, would have silently missed the two real, actionable D2/D4 gaps and mislabeled them as accepted risk instead of fixed. (2) Fix only the 2 confirmed-real failures without re-establishing true baseline ground truth first — rejected, the 33-vs-36 discrepancy needed to be resolved with evidence before any RISK-ACCEPT or "no regressions" claim could be made honestly.
**Rationale:** This is exactly the class of thing CLAUDE.md's own "verify coding-agent dispatch completion independently — do not trust the agent's self-report" guidance generalizes to: a citation inherited from an earlier phase of this same session (not a subagent this time) turned out to be inaccurate, and it was only caught by attempting to actually open the cited entry rather than assuming it existed because the plan document referenced it confidently.
**Made by:** Claude (agent), via a disposable worktree at the exact baseline commit rather than trusting the cited record
**Revisit trigger:** None — `feature/wsi-s1` is now confirmed at true baseline parity (33/33, identical file list) and ready for `/verify-completion`.
---
