...
**AC3:** Given a credential that is not authorized for the product owning the requested feature slug, When the request is made, Then the endpoint returns a 403 (matching the existing `ExportAccessDeniedError` behaviour) without revealing whether the slug exists in a different tenant's repo — the error response must not differ observably between "slug doesn't exist anywhere" and "slug exists but you can't access it."
