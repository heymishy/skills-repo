...
**Post-merge live Chrome verification (backfilled 2026-09-11 — see DoD Observation #5):** Performed on both `wuce-staging.fly.dev` and production (`skills-framework.fly.dev`) after deploy. Confirmed directly via browser automation: `window.scrollY === 0` and `document.activeElement === document.body` on a plain `/journey` load with the real, live 269-feature list (not the E2E spec's synthetic 40-journey fixture) — the exact production-scale case the original bug report described. This is independent, live confirmation beyond the automated E2E suite, against the real dataset size the bug was originally found at.
...
5. **Backfilled 2026-09-11:** a repo-wide stocktake of DoD verification methods found that live Chrome verification (staging and production) had actually been performed for this story immediately after merge, in the same session — but the confirmation was never written back into this artefact, existing only in conversation history until now. Logged as a process gap at `/improve`: live-verify, then immediately update the DoD before moving to the next task, rather than letting it be captured only in chat.
