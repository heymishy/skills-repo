...
**AC3:** Given the operator is in a reopened stage's live session, When they load `GET /api/journey/:id`, Then `completedStages`, `stage`, and `stages[]` in the response are unchanged from before the reopen — the journey's forward-progress record is not corrupted or duplicated.
