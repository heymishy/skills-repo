...
**AC2a (added once AC1's investigation is complete, before implementation):** Given the confirmed real source signal for per-feature health (identified via this story's Architecture Constraints investigation), When a feature's health is computed, Then it matches that confirmed rule exactly — this AC is intentionally left to be written concretely once the investigation resolves; it must not be skipped or left as AC2's provisional inequality alone.
