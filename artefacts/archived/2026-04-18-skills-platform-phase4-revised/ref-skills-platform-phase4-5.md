...
3. **Second-line organisational independence** — the assurance agent's SKILL.md is controlled by the same platform team that owns the delivery agents. For the regulator's operational-risk policy purposes (e.g., RBNZ BS11), this is not credible organisational second-line independence. The platform can structurally support independence; achieving it is an organisational control.
...
**1a — Repo structure collision.** The consumer's repo already owns `.github/`, `.gitignore`, existing conventions, possibly a `CLAUDE.md` or agent instruction file, its own standards. The installer wants to write into `.github/skills/`, `.github/standards/`, `.github/workflows/`, `.github/templates/` — any of which may collide with existing content. Observed: multiple the consumer organisation squads blocked from adoption because clean install isn't possible without destroying existing structure.
...
> The assurance agent's instruction set (its SKILL.md) is authored, controlled, and approved by the platform team — the same team that owns the first-line delivery agents. True organisational second-line independence requires the second-line function to be independent from what it's assessing. A regulator examiner (e.g., RBNZ, HQSC) will ask: who governs the assurance agent's instruction set, and are they independent of the delivery function? The honest answer is currently no.

*What breaks at scale:* At 5 squads in a dogfood context this limitation is acceptable with documentation. At 50 squads in a regulated enterprise (e.g., a bank under RBNZ BS11), the second-line framing is a regulatory posture, not an architectural one. An examiner will probe this.
...
- 4.B.3 — Configurable commit format validation: governance package supports operator-configured commit message format (consumer-organisation example: SPPX-XXXX Jira story reference; each consumer applies their own format) as structural precondition before advancing pipeline state. Format rules live in `context.yml`, not hardcoded.
...
- 4.B.6 — Upstream authority model: resolves the P4.4 open question from PR #98 — is `heymishy/skills-repo` the authoritative upstream for the consumer organisation consumption, or does the productisation fork remain the publishing source? Phase 4 cannot complete Theme B without this decision.
...
*Audit evidence accessibility (non-technical readers):*
- 4.B.9 — CI-native individual artefact attachment: at assurance gate pass, CI uploads the full artefact chain as individually named files (00-summary.md through 07-trace.jsonl) to the CI artefact store. An issue tracker post links to the summary. A non-technical operator or auditor reaches the full chain from the CI run or the issue tracker in two clicks — no git access, no tooling, no training. Storage footprint is approximately 100 KB per feature at current artefact sizes, scaling to approximately 300 MB per year at 50 teams. Built as a `--collect` extension to trace-report.js (already a zero-dependency Node script that walks the artefact chain). Consumer configures in `context.yml` under an `audit:` block (default off, opt-in per consumer).
...
*Scope:* The platform cannot unilaterally achieve organisational independence, but it can structurally support it. Theme F delivers the governance and review controls that make credible second-line independence claims possible to the regulator.
...
- 4.F.3 — regulator-ready documentation: operating model §6 limitation documented as closed, with evidence chain
- 4.F.4 — CI artefact chain on external platform: the assurance gate's CI-native artefact attachment (4.B.9) places the full evidence chain on a platform outside the delivery team's repository. When combined with the issue tracker post, the second-line reviewer accesses governed evidence without requesting repository access or learning git tooling. This supports the organisational independence claim: evidence is produced by the delivery pipeline, stored on a shared CI platform, and accessible to the risk function through a familiar interface.
...
**B2 — CLI enforcement mechanism and PR #98 fit.** Is craigfo's CLI MVP the right adapter shape for the governance package (Spike A output), or does the package architecture require a different CLI structure? Specifically: can the CLI verify enterprise-traceability-compliant commit message format (e.g. the consumer organisation's Jira story reference requirement, SPPX-XXXX format) as a structural precondition before advancing pipeline state? Commit format validation should be operator-configured per consumer's internal traceability standard, not hardcoded — other consumers will have differently-shaped requirements. This is the concrete enforcement response to distribution sub-problem 1b.
...
**C5 — Artefact collection and story identification for CI attachment (intersects 4.B.9).** Two questions. First: how does the assurance gate identify which feature's artefacts to collect when a PR may contain commits from multiple stories? The gate currently runs trace-report.js which resolves features from pipeline-state.json — the `--collect` extension inherits this resolution, but the mapping from PR commits to governing story needs explicit design. Second: how does the issue tracker post identify the governing story from the commit trail — does it rely on commit message format (operator-configured per 4.B.3) or on pipeline-state.json story-to-PR association?
...
*Output:* Distribution architecture with a specific design response per sub-problem, tradeoffs named, migration path from Phase 3 installs. Must address: update propagation to non-git consumers, audit chain integrity when skills are fetched rather than vendored, conservative pinning without channel severance, consumer-organisation enterprise network and security constraints, auditor discoverability of governance content in a sidecar install.
...
4.F — Second-line organisational independence. Governance model, approval authority separation, regulator-ready documentation.
...
**5.R2 — Agent identity layer:** Signed identity per agent execution traceable to model version and instruction-set version. Combined with Theme F second-line independence, this closes the regulator audit chain.
...
2. **C11 at persistent-store scale.** The 5.H cross-team trace registry must resolve the ADR-004 tension. If the chosen resolution creates operational costs that warrant revisiting C11, requires a formal ADR with regulator implications assessed.
...
**C1 + C4 + C11 — alignment with 4.B.9 (CI-native artefact attachment).** The CI attachment model preserves C1 (update channel) because artefact collection runs through the same trace-report.js that already walks the governed chain — no separate tool or channel required. C4 (human approval gate) is strengthened: the non-technical reader sees the same governed evidence the gate evaluated, on a platform they already have access to. C11 (no persistent agent runtime) is satisfied because artefact collection and issue tracker posting run as CI steps within the existing assurance gate workflow — no new hosted service.
...
| 2026-04-19 (v7) | **CI-native artefact attachment replaces audit snapshot.** 4.B.9 added: at assurance gate pass, CI uploads individually named artefact files to CI artefact store and posts summary link to issue tracker — non-technical reader reaches full chain in two clicks with no git access. Theme F extended with 4.F.4 (CI artefact chain supports second-line independence on external platform). Spike C extended with C5 (artefact collection resolution and story identification from commit trail). Constraint alignment note added for C1 + C4 + C11 against 4.B.9. Supersedes v6 draft (PR #173, audit snapshot push adapter — replaced with simpler CI-native approach) |
