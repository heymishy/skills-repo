# Corrected Stories — Impact/Effort Matrix Workshop Tool
## All findings from Review Run 1 applied

**Feature slug:** 2026-06-04-impact-effort-matrix-workshop
**Corrected:** 2026-06-04
**Based on:** Review Run 1 findings 1-M1 through 1-M7, 1-E1, 1-L1, 1-L2

**Save individual files to:** `artefacts/2026-06-04-impact-effort-matrix-workshop/stories/`
...
| Finding | Change |
|---------|--------|
| 1-M1 | References section added to all 7 stories (discovery.md + benefit-metric.md paths) |
| 1-M2 | NFRs field added to all 7 stories; iem.5 performance NFR moved from Architecture Constraints to NFRs field |
| 1-M3 | iem.2 AC3 rewritten: removes localStorage key reference; asserts observable page-refresh behaviour instead |
| 1-M4 | iem.3 AC1 simplified: compound "and the card's impact and effort coordinates in session state update" clause removed |
| 1-M5 | iem.4 AC1 rewritten: defines .txt segmentation rule (blank-line split, single segment fallback) |
| 1-M6 | iem.5 AC1 and AC3 rewritten: localStorage key name and `segmentLinks[]` references removed from Then clauses |
| 1-M7 | iem.6 AC2 rewritten: `segmentLinks[]` reference removed; AC4 rewritten to assert page-refresh observable behaviour |
| 1-E1 | iem.3 Architecture Constraints: Playwright dual-server configuration constraint added |
| 1-L1 | Complexity field added to all 7 stories |
| 1-L2 | Scope stability field added to all 7 stories |

---

## iem.1 — Application shell and 2×2 canvas

**File:** `stories/iem.1-application-shell.md`
...
#### References
- Discovery: `artefacts/2026-06-04-impact-effort-matrix-workshop/discovery.md`
- Benefit metric: `artefacts/2026-06-04-impact-effort-matrix-workshop/benefit-metric.md`
...
**File:** `stories/iem.2-card-creation.md`
**Epic:** iem-live-session
**Stage:** definition
...
Given I have added one or more cards in the current session
When I refresh the page
Then all cards are displayed at their last-saved names, labels, and positions
...
**File:** `stories/iem.3-card-positioning.md`
**Epic:** iem-live-session
**Stage:** definition
...
Then the card moves to and stays at the new location

Given a card is on the canvas
When I select it by keyboard (Tab to focus) and press an arrow key
Then the card moves in the direction of the arrow key press
...
#### Scope stability
Unstable (Playwright dual-server configuration for Vite dev server not yet validated in this repo)
...
- **Playwright dual-server configuration (1-E1):** The root `playwright.config.js` is extended with a second `webServer` entry targeting the Vite dev server (`npm run dev` inside `workshop-tool/`, serving on a distinct port from the Node.js web-ui server). Workshop-tool E2E specs live under `tests/e2e/workshop-tool/`. A `decisions.md` entry documents this as a scope clarification of ADR-018 (the workshop-tool Vite dev server is a new surface type; `tests/e2e/workshop-tool/` is the established spec path convention for it).
...
**File:** `stories/iem.4-transcript-import.md`
...
Then the application parses the file into segments split on blank lines (treating the entire file as one segment if no blank lines are present), displays the parsed segment count, and the "Auto-segment" button becomes enabled
...
#### Scope stability
Unstable (.txt blank-line segmentation rule is new — not yet validated against real Zoom/Teams/Loom export format samples)
...
**File:** `stories/iem.5-auto-segmentation.md`
**Epic:** iem-post-session
**Stage:** definition
...
Given cards exist on the canvas and a transcript has been imported, and no Claude API key has been set
When I click "Auto-segment"
Then an inline API key entry prompt appears; when I enter a key and confirm, the segmentation call begins immediately
...
Then the card detail panel for each card shows its linked segment excerpts with start time, end time, and a brief relevance reason
...
#### NFRs
Call completes within 30 seconds for a typical 60–90 minute transcript at standard verbosity; the call is abandoned after 60 seconds (hard timeout).

#### Complexity
3

#### Scope stability
Unstable (Claude API response shape not yet validated against a real transcript; model selection not confirmed)
...
**File:** `stories/iem.6-segment-review.md`
**Epic:** iem-post-session
**Stage:** definition
...
Then that segment link is no longer shown in the card detail panel
...
Given I have dismissed or manually added segments and then refresh the page
When the page reloads
Then the card detail panel for each card displays only the accepted segment links, with dismissed links absent and manually added links present
...
**File:** `stories/iem.7-markdown-export.md`
**Epic:** iem-post-session
**Stage:** definition
