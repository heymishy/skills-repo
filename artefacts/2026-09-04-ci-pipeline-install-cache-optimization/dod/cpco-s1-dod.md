...
3. **Follow-up action #1 (watch the next real staging-deploy run) was completed the same day and produced a genuinely mixed real-world result, reported here in full rather than only the favourable half.** In the first real `staging-deploy.yml` run after this story's own merge (run `33855992193`, triggered by this story's own merge commit): `deploy-staging`'s "Install dependencies" step took exactly 7m01s -- identical to the pre-fix baseline -- despite `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD: 1` being confirmed present in the step's own logged env, a confirmed warm npm-cache hit (`Cache hit for: node-cache-Linux-x64-npm-...`), and zero browser-download activity anywhere in the log -- the entire 7 minutes was silent (no output at all between one deprecation warning and `added 235 packages in 7m`). This is not attributable to this story's own change (the postinstall skip demonstrably worked -- no browser download occurred), and is most likely an isolated npm-registry/network stall, not a regression. Confirmed by contrast in the very same run: `smoke-test`'s own "Install dependencies" (same lockfile, same commit, running two minutes later) took 50 seconds, and "Install Playwright browsers (Chromium only)" took 22 seconds with the new cache step reporting a hit -- both squarely in line with this story's own claimed improvement. Real-world evidence is therefore 3-for-4 unambiguously fast (PR #830's own 4 CI jobs) plus 1-for-2 fast on the first real staging-deploy run (`smoke-test` fast, `deploy-staging` anomalously slow) -- worth one more observed staging-deploy run before treating `deploy-staging`'s own install time as fully confirmed, but not worth reopening this story for, since the anomaly's own log evidence points away from this story's own change as the cause.
