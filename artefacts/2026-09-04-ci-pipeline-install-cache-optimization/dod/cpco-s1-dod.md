...
3. **Follow-up action #1 (watch the next real staging-deploy run) was completed the same day and produced a genuinely mixed real-world result, reported here in full rather than only the favourable half.** In the first real `staging-deploy.yml` run after this story's own merge (run `33855992193`, triggered by this story's own merge commit), all 3 of its jobs that run `npm ci` were checked directly against their own raw logs:
   - `deploy-staging`: "Install dependencies" took 7m01s (identical to the pre-fix baseline).
   - `smoke-test`: "Install dependencies" took 50s; "Install Playwright browsers (Chromium only)" took 22s with the new cache step reporting a hit.
   - `post-deploy-e2e-confirm`: "Install dependencies" took 6m27s (`added 235 packages in 6m`); "Install Playwright browsers (Chromium only)" took ~17s, also a confirmed cache hit.

   In every one of the 3 jobs, `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD: 1` was confirmed present in the logged step env, `npm`'s own cache reported `Cache hit for: node-cache-Linux-x64-npm-...` every time, and zero browser-download activity appeared anywhere in any of the 3 logs -- this story's own actual scope (skip the postinstall browser download; cache Chromium for jobs that need it) is unambiguously working in all 3 cases, including the 2 slow ones. The slowness in `deploy-staging` and `post-deploy-e2e-confirm` is a *different*, newly-surfaced pattern: raw `npm ci` itself (`added 235 packages in ...`) varies from 50s to 7 minutes across 3 jobs sharing the identical lockfile and an identically-reported cache hit, with total log silence during the slow runs (no per-package output, no registry errors, nothing between one deprecation warning and the final `added N packages` line). One suspicious data point: the restored npm cache size was reported as only ~42MB, which seems small for 235 packages' worth of tarballs -- worth checking whether the cache is being saved/restored completely, since a cache reporting "hit" without holding every needed tarball would still force `npm` to fall through to the registry per-package, which could explain both the multi-minute silence and the inconsistency between jobs. **This is a real, separate finding, not a defect in this story** -- filed here rather than acted on immediately, as a candidate for a future dedicated investigation (npm-cache completeness/effectiveness on this repo's own CI, not a Playwright-specific issue) rather than reopening this story's own narrower scope.
