...
**Follow-up actions:**
1. **Approve `promote-to-prod`** in GitHub Actions for this merge commit (`724eaa67`) whenever convenient — `wuce-staging.fly.dev` deploy + smoke test both passed (workflow run `33797888904`); `skills-framework.fly.dev` needs its own separate approval, same recurring deploy-topology pattern noted in every prior DoD this session. Not time-sensitive — this is a read-path bug fix restoring visibility of already-recorded artefacts, not a data-loss risk. Owner: Hamish King.
