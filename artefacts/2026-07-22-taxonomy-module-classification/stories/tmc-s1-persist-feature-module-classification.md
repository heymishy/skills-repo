...
**AC5 (rendering, module-grouped taxonomy):** Given a product's taxonomy (from `computeTaxonomyRollup`) and its feature-module assignment map, When the product view renders, Then taxonomy items are grouped by their assigned module (module name as heading) with a distinct "Unclassified" bucket for any feature with no assignment row — and a product with zero assignments renders **exactly the existing, unchanged taxonomy/epic-phase grouping** (no visual regression for products that haven't adopted module classification yet).

**AC6 (module deletion reassigns, doesn't orphan):** Given a module with existing feature assignments is deleted, When `deleteModule` runs, Then every `feature_module_assignments` row referencing it is explicitly set to `module_id = NULL` (Unclassified) via an explicit `UPDATE` issued *before* the module row is deleted — matching the existing `journeys.module_id` reassignment-before-delete convention in the same file — never left orphaned or silently cascade-deleted.
...
- Any change to `journeys.module_id` or the existing a2 epic-reassignment UI/flow — that mechanism is untouched and continues to work for placeholder/manually-created journeys exactly as today.
