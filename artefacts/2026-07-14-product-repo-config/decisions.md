...
**2026-07-15 | SCOPE | prc-s2.1 implementation — reconciled with prc-s1.2's real (merged) adapter, revisit trigger closed**
**Decision:** `prc-s1.2` merged to `origin/master` (PR #479) partway through this story's implementation. Rebased `prc-s2.1`'s branch onto `origin/master` and discarded the speculative `Object.assign`-merged, object-of-methods `repoAdapter` shape built earlier the same session (see the prior entry above), reconciling instead against `prc-s1.2`'s actual shipped adapter: a bare-function `_repoAdapter`/`setRepoAdapter`/`getRepoAdapter` (not an object of methods). Added a second, independently-named pair — `_createRepoAdapter`/`setCreateRepoAdapter`/`getCreateRepoAdapter` — to the same `src/web-ui/adapters/repo-adapter.js` file, following `prc-s1.2`'s exact bare-function convention rather than my own earlier `Object.assign` design. `prc-s1.2`'s own exports (`setRepoAdapter`, `getRepoAdapter`, `realCheckRepoAccess`) are untouched.
**Alternatives considered:** Keep the earlier `Object.assign`-based design and let the orchestrator resolve the merge conflict later (rejected — now that the real dependency is visible, resolving it correctly here is strictly better than leaving an admittedly-speculative, now-known-wrong design for someone else to untangle blind; the orchestrator's "don't worry about the overlap" instruction was reasonable when prc-s1.2's shape was unknown, but ceases to be the right call once the actual shape is available in this same session). Modify `prc-s1.2`'s existing `_repoAdapter` to hold an object of methods instead of a bare function, to match my original design (rejected — this would require also editing `product-repo.js`'s already-shipped, already-tested `handlePostConnectRepo`, which calls `getRepoAdapter()(owner, repo, accessToken)` directly as a function; that is scope creep into a merged, unrelated story's code for no behavioural benefit).
**Rationale:** Two independently-wired, distinctly-named D37 adapters in one file — one per story, one per capability (verify-access vs. create) — satisfies the DoR contract's original intent ("not a second, parallel adapter *module*") without forcing either story's capability into an artificial shared shape. `node tests/check-prc-s1.2-connect-repo.js` (9/9 passing) and `node tests/check-prc-s2.1-create-repo.js` (6/6 passing) both pass against the reconciled file with zero cross-story regression.
**Made by:** Claude (agent), via inner coding loop (post-implementation rebase), 2026-07-15.
**Revisit trigger:** None — this closes the revisit trigger from the prior entry (2026-07-15, "DoR contract assumption did not hold"). The reconciliation described there (two module definitions merging into one) is complete and verified in this branch.
