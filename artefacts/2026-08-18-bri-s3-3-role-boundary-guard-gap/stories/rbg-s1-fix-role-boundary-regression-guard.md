...
**AC2:** Given AC3's currently-placeholder test body, When implemented, Then it uses the existing `VIEWER_PERSON_ID` fixture to assert the specific role-boundary condition AC3 was originally scoped to cover (per the story's own AC3 text) — replacing the placeholder with a real assertion, not another no-op.
