...
- **Mirror `requireAdmin`'s existing pattern** (`src/web-ui/middleware/require-admin.js`) — fail-closed by default, checks `req.session.role` with a live-role re-check via an injectable adapter (`setGetCurrentRole`) when wired, self-heals the cached session role, and logs every denial. The new gate (`requireNonViewer` or equivalent) must reuse the *same* live-role-resolution call, not duplicate it — both gates need to observe the same live role on the same request.
- **Refactor `require-admin.js` to export a shared role-resolution helper** (e.g. `resolveRole(req)`), used by both `requireAdmin` and the new `requireNonViewer` gate, as a distinct implementation task from writing the new gate itself (decided via `/decisions`, 2026-08-22, ARCH entry — resolves review finding `1-M1`). Do not duplicate the session-role-read + live-role-check logic in the new gate file — the two gates must observe identical role-resolution behaviour by construction, not by two independently-maintained copies.
