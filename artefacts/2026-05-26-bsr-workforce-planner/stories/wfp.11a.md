...
**Data model correction applied:** User Story updated (teams are the primary assignable unit); Dependencies updated (`scoreTeamForSlug` replaces `scoreSquadForSlug`); AC2 updated (GET /workforce/data now also returns `teams` key from teams.json); AC4 updated (initiative-centric view lists teams as primary candidates with individual members as a drill-down); AC5 updated (add/remove operates on teams, not individual people, as primary action); AC8 updated (`scoreTeamForSlug` replaces `scoreSquadForSlug`; coverage is union of member skills, not per-person); Complexity Rationale updated.

## User Story

As a **Head of Engineering**,
I want an interactive browser UI within the existing web UI where I can view all portfolio initiatives, see ranked candidate teams (with individual members as a drill-down) for each, and author or update `workforce/allocation-input.json` without hand-editing JSON or running terminal commands,
...
- **Upstream:** wfp.10 (skill-tag match scoring) must be DoD-complete — the UI reuses the same scoring logic to rank candidate teams per initiative. The wfp.10 DoR contract (`artefacts/2026-05-26-bsr-workforce-planner/dor/wfp.10-dor-contract.md`) confirms that `scoreTeamForSlug(team, requiredTags)` is exported from `src/workforce/assign.js` as a named function. The `src/web-ui/routes/workforce.js` handler must `require` and call `scoreTeamForSlug` directly — do not reimplement the scoring formula inline. This dependency is resolved; no extraction task is needed.
...
**AC4 (initiative-centric view — default on load):** Given the assignment UI has loaded and `GET /workforce/data` returns non-empty `portfolioSlugs`, when the page renders, then the initiative-centric view is displayed by default. The left panel lists all portfolio initiatives, each showing its slug, product group, and allocation status derived from `allocationInput`: "unassigned" if no entry exists for the slug; "partially assigned" if an entry exists with no `teamId` and an empty or missing `people` array; "fully assigned" otherwise. Initiatives are filterable by product group and by allocation status using controls above the list. Selecting an initiative shows: its `requiredTags` and `people.fte_demand` from the portfolio slug data; current team and person assignments from `allocationInput`; a ranked candidate list (see AC8); and a coverage indicator showing the percentage of `requiredTags` covered collectively by all currently assigned teams (union of member skills across all assigned team entries).

**AC5 (initiative-centric view — add/remove and allocation mode):** Given the initiative-centric view is active and an initiative is selected, when the operator adds a team as an assignee, then the change is staged in memory and reflected immediately in the UI without writing to disk. Teams are the primary assignable unit: the team entry (with its `teamId`) is written to the staged allocation. The operator can optionally expand a team to select named override individuals from within that team. A per-initiative allocation mode selector supports "direct", "profile-match", and "net-new". Selecting "net-new" reveals `requiredRole` and `requiredTags` input fields. The operator can remove any existing team or individual from the initiative. The staged allocation entry also exposes `parentSlug` and `scopeLabel` fields (for wfp.8 rollup decomposition). No data is written to disk until the operator clicks "Save assignments".

**AC8 (candidate ranking by skill-tag match score):** Given the initiative-centric view is showing an initiative that has a non-empty `requiredTags` array, when the candidate list is rendered, then teams from `workforce/teams.json` are ranked by their team-level skill-tag match score against `requiredTags`, computed using `scoreTeamForSlug(team, requiredTags)` from `src/workforce/assign.js` (coverage formula: `coveredTags.length / requiredTags.length` where `coveredTags` is the intersection of `requiredTags` with the union of all member `skills` arrays for the team, resolved via `roster`). The match score is displayed as a percentage alongside each team (e.g. "84% match"). Teams with a score of 0 are shown below a "No tag match" separator and are not hidden. Each team entry in the candidate list is expandable to show its individual members, with individual person match scores displayed as a secondary drill-down.
...
**Rating:** 3
**Rationale:** Most of the structural complexity of the full wfp.11 story — four route handlers, atomic file writes, child-process execution, path traversal protection, team-level candidate ranking via `scoreTeamForSlug`, and the initiative-centric view with staged in-memory state. The `GET /workforce/data` response now includes `teams` in addition to `roster` and portfolio data. Vanilla JS single-file HTML requires careful coordination between the route module and the inline script. Story B (wfp.11b) is lower risk because the route layer is already established.
**Scope stability:** Stable. The `scoreTeamForSlug` export is confirmed in the wfp.10 story correction. Four routes and their handler names are defined and agreed. Initiative-centric view spec is complete.
