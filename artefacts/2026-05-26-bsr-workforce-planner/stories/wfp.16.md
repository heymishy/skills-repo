...
- `workforce/initiative-map.json` — included when the query contains any of the following keywords (case-insensitive substring match): "initiative", "allocation", "fte", "assigned", "coverage", "map", "bottleneck", "over-committed", "over-allocated", "at risk", "capacity", "assign", "portfolio". This keyword list is a known Phase 2 approximation — queries phrased without these words (e.g. "where are the pressure points?") will not trigger Tier 2 automatically. The chat UI includes a manual "Include allocation data" toggle (unchecked by default) that forces Tier 2 inclusion regardless of keyword detection. When the toggle is active, the `tiersUsed` response field includes `"T2"` even if no keywords matched.
...
**AC1 (route registration — GET /workforce-chat):** Given an authenticated operator navigates to `GET /workforce-chat`, then the server returns `200 text/html` containing a chat interface. The interface includes: a message input field, a send button, a streaming response area, an "Include allocation data" toggle (unchecked by default, forces Tier 2 regardless of keyword detection), an "Include full roster details" toggle (unchecked by default, opt-in for Tier 3), and a nav link back to the intelligence dashboard. The chat HTML is an inline single-file response — no external CSS or JS files.

**AC2 (route registration — POST /api/workforce-chat/turn, non-streaming):** Given an authenticated `POST /api/workforce-chat/turn` with body `{ "message": "...", "history": [...], "includeFullRoster": false, "includeTier2": false }`, when the handler processes the request, then it assembles the context window (Tier 1 always; Tier 2 if message keywords match or `includeTier2: true`; Tier 3 only if `includeFullRoster: true`), calls `_workforceQueryExecutor` with the assembled prompt and conversation history, and returns `200 application/json { response: "...", tiersUsed: ["T1"] }`. If `_workforceQueryExecutor` throws, the response is `500 application/json { error: "Workforce query failed. Check server logs." }` — raw error messages are not forwarded to the client.
...
**AC5 (D37 production wiring verification):** Given `NODE_ENV !== 'test'` and the server has started via `npm run workforce`, when a test inspects the module state after server boot, then `_workforceQueryExecutor` and `_workforceQueryExecutorStream` are not the default stub functions (i.e. they have been replaced by `setWorkforceQueryExecutorAdapter` and `setWorkforceQueryExecutorStreamAdapter` respectively in `server.js`). The verification mechanism: the route handler module exports `__getAdapters()` guarded by `if (process.env.NODE_ENV === 'test')`, returning `{ executor: _workforceQueryExecutor, streamExecutor: _workforceQueryExecutorStream }`. A test uses `require('../routes/workforce-chat').__getAdapters()` after server initialisation and asserts both values are not the default stub functions. This is the only approved mechanism — subprocess server-start tests are not required.
...
**AC7 (Tier 2 on-demand inclusion):** Given a `POST /api/workforce-chat/turn` body where either (a) `message` contains at least one of the Tier 2 keywords ("initiative", "allocation", "fte", "assigned", "coverage", "map", "bottleneck", "over-committed", "over-allocated", "at risk", "capacity", "assign", "portfolio") or (b) `includeTier2: true` is set in the request body (set by the manual "Include allocation data" toggle in the UI), when the context window is assembled, then `workforce/initiative-map.json` content is included. If `initiative-map.json` does not exist, a note is added to the context: "Note: initiative-map.json is not present. Run workforce-map to generate it." If `workforce/allocation-input.json` does not exist, it is silently omitted from the Tier 2 context with no error (same fallback pattern as absent `initiative-map.json`). The `tiersUsed` response field includes `"T2"`. A test verifies that `includeTier2: true` alone (without any keyword match) triggers Tier 2 inclusion.
