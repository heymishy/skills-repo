...
**AC2 (bottlenecks-data JSON — skill bottlenecks):** Given `workforce/initiative-map.json` exists, when I call `GET /api/intelligence/bottlenecks-data`, then the response includes a `skillBottlenecks` array. The **tag universe** for bottleneck analysis is exclusively the union of all `requiredTags` arrays across all portfolio slugs referenced in `initiative-map.json` — it is NOT the union of all skills in `roster.json`. Only tags the portfolio actually requires are evaluated for concentration risk; skills held by team members that no current initiative requires are excluded. Each entry contains: `tag` (the skill tag string), `teamCount` (number of distinct allocated teams that have at least one member holding this tag), and `teams` (array of `{ teamId, teamName, memberCount }` for those teams). Only tags where `teamCount` is less than or equal to 1 (i.e. the tag exists in at most one team's collective skills across all allocated teams) appear in this array. Tags with `teamCount: 0` (no allocated team covers a required tag) are included with `teams: []`. The array is sorted by `teamCount` ascending, then alphabetically by `tag`. A `tagUniverse` field at the response root lists all distinct `requiredTags` values used as the analysis universe, for auditability.
