...
**Update (backfilled 2026-09-11 — see DoD Observation #5): the walkthrough ran, and the metric can now be marked measured.** The deferred walkthrough this row names as "the one remaining action" **was executed on 2026-09-09**, the same day this DoD was written, later in the session — full detail in `artefacts/2026-09-08-session-origin-badge/decisions.md`'s "Post-merge verification walkthrough executed (RISK-ACCEPT W4 closed)" entry. Combined-pass result across all 3 surfaces: `skills-framework` product list 630 live badges (9 fully-session-backed, 621 no-session), `/journey` 264 live cards (6 fully-session-backed, 1 real mixed card, 257 no-session), org kanban 31 live cards (all fully-session-backed, 0 no-session — matching the documented structural limitation that org kanban's journeys-only query cannot represent a zero-journey row). That closure was logged in `decisions.md` but never written back into this DoD. **Independently re-confirmed fresh on 2026-09-11**, `getComputedStyle`-verified (not just `className`): product list 630/630 unchanged, `/journey` 269 (organic growth from 264, same 1 mixed / 6 fully-session-backed split), org kanban 31/31 unchanged — all badges carrying a real resolved `backgroundColor`, not just the class string.

**Metric Signal, corrected:** `List-view session-origin visibility` should read **Measured, 100%** as of 2026-09-09 (live-confirmed), not `not-yet-measured` as originally recorded above.
...
1. ~~Execute the RISK-ACCEPTed post-merge manual verification-script walkthrough~~ — done, 2026-09-09; independently re-confirmed 2026-09-11 (see Metric Signal update above).
2. ~~Complete `journey.js`'s own half of the deferred DRY extraction~~ — done, `sob-s4`.
3. ~~Update `nfr-profile.md`~~ — done, 2026-09-11.
4. ~~Add a tone modifier class (e.g. `sw-pill--neutral`)~~ — done, `sob-s4` (which itself found and fixed a real gap the original walkthrough missed — see DoD Observation #5).
5. Two genuine tool/process defects surfaced during this feature's delivery are worth a dedicated `/improve` pass: (a) `bin/skills advance`'s dot-notation silently corrupts array-shaped fields like `tasks[]` (see DoD Observations below); (b) a PR with a real merge conflict against master shows "no checks reported" rather than surfacing the actual blocker, easy to misread as "CI just hasn't started yet." Neither actioned as of this backfill (2026-09-11) — still open, genuine `/improve` candidates.
...
5. **Backfilled 2026-09-11, following a repo-wide DoD-verification-method stocktake:** this DoD's own Metric Signal section — the feature's closing story, and the exact point the deferred walkthrough could finally run — said the walkthrough "has not yet been executed... is the primary remaining action" when written. It ran that same day, later in the session, and its real, positive result (all 3 surfaces at 100%, one real defect found by the immediate follow-up `sob-s4` and fixed the same day) sat undocumented in this artefact for two days until this stocktake caught it. Same root pattern as `sob-s1`/`sob-s2`'s own DoDs and `wnl-s2`'s (a sibling feature, same session): live verification that actually happened is only as durable as the artefact it gets written into — chat history and even `decisions.md` are not a substitute for updating the DoD itself, since the DoD is what `/trace` and a future reader check first.