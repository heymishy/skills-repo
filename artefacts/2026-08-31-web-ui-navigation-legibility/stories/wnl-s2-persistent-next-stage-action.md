...
- Reuse the existing, already-live `position: sticky` pattern from `src/web-ui/utils/html-shell.js:568` (`.sw-imp-banner`, the impersonation banner) — same underlying CSS technique, anchored to `bottom: 0` instead of `top: 0` (natural for a "continue forward" action placed at the end of a chat). Do not introduce a new positioning mechanism, a JS-based scroll listener, or a new npm dependency (`product/tech-stack.md`'s "Zero new npm dependencies" runtime constraint and Mandatory Constraint MC-SELF-02 — not ADR-009, which governs an unrelated topic, CI evaluation/write-back trigger separation; see `CLAUDE.md`'s own note on this exact prior mis-citation).
...
- **Existing test dependency:** `tests/check-lsbm-s1-live-substep-injection.js` (lsbm-s1) uses the literal string `'<div class="sw-journey-gate"'` as a slice-boundary anchor (lines 284, 299) to extract a portion of the rendered chat HTML for its own assertions. The `class="sw-journey-gate"` attribute must remain that exact literal substring at the start of the div's opening tag — add sticky positioning via the existing `style="..."` attribute (already present on this div) or an additional stylesheet rule targeting `.sw-journey-gate`, not by restructuring the div itself or wrapping it in a new outer container. Run `node tests/check-lsbm-s1-live-substep-injection.js` before considering this story done, not assumed passing.
