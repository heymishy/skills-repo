...
**AC2:** Given a story's pipeline-state entry has `hasDesignSystemTrack: true`, When `/definition-of-ready` runs its hard-block checks, Then a new `H-DESIGN` block scans the story's declared touched files for hardcoded color values not present in `DESIGN.md`'s token table, and fails with a specific message naming the offending file and value if any are found — matching the exact fail-message format convention of existing hard blocks (e.g. `H-ADAPTER FAIL: ...`). (Corrected 2026-09-20, see `decisions.md`: this AC originally also named "font" values; font-value governance was never scoped by the test plan and DESIGN.md's Typography section has no structured, regex-extractable token table comparable to its color-token table, so it was never buildable within this story's own Complexity-2 scope. Text corrected to match what was actually specified, tested, and delivered — color values only.)
...
**AC4:** Given a compliant test story tagged `hasDesignSystemTrack: true` with only token-table color values in its declared touched files, When `/definition-of-ready` runs, Then `H-DESIGN` passes and does not block sign-off — proving no false positive.
