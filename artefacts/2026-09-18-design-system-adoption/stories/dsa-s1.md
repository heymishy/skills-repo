...
- Confirmed via `/clarify`: reuse and extend `src/web-ui/utils/html-shell.js`'s existing CSS custom-property blocks — do not introduce a parallel styling mechanism. **Real structure, confirmed via direct read (corrects an earlier, imprecise description in this story's own DoR/test plan): bare `:root` is LIGHT mode (the default, no attribute needed), `[data-theme="dark"]` on `<html>` (set by a real JS toggle, `html-shell.js:269-287`) is the DARK override, plus a no-JS `@media (prefers-color-scheme: dark)` fallback block guarded to `:root:not([data-theme="light"]):not([data-theme="dark"])`.** Update hex values in both `:root` (light) and `[data-theme="dark"]` (+ its `@media` fallback twin) to match `DESIGN.md`'s light/dark token tables respectively. Two tokens (`--surface-2`, `--muted-3`) don't exist yet in either mode — must be added new, not just updated. The current font-family is `'Inter'` (`--sans`); `DESIGN.md` specifies `'Inter Tight'` for UI/display — this is a real, additional change beyond color tokens, update the `--sans` value too.
