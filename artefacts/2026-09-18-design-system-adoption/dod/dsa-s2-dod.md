...
6. **Mobile responsiveness gap (found post-merge, in response to a direct operator question)**: the new dashboard content (`.sw-skill-grid`'s fixed 3-column grid, `.sw-cols`' fixed 2-column grid) has no `@media` breakpoint and genuinely overflows horizontally below 768px (confirmed via Playwright at 375px/390px viewports: 140px of real horizontal overflow, cramped skill-card text wrapping). The shared shell/sidebar IS responsive (inherited from `dsa-s1`, confirmed clean at both widths on the zero-products path). Not a regression — neither this story's ACs nor `DESIGN.md` specified mobile behavior for this screen — but a real gap now shipped to production. See `decisions.md`'s dedicated entry for full evidence. Candidate fix: `auto-fit`/`minmax()` responsive grid or an explicit `@media (max-width: 768px)` stack-to-1-column override. No owner assigned.
