...
**Assessed by:** Copilot
**Date:** 2026-09-19 (amended 2026-09-19, same day: live browser render check completed against real staging — see below)
...
| AC5 | ✅ | Same spec's two mobile-viewport tests (375px/390px), fresh run: passed. Real overflow bugs were found and fixed empirically during implementation (a long SHA-256 hash forcing overflow, header actions not fitting at 320px) — not just claimed clean. Mobile portion of the live browser check attempted against real staging but not achievable (browser-window-resize tool does not reach a true 375-390px width in this environment); Playwright evidence remains this AC's primary verification. | live-verified | None |

**A deviation is any difference between implemented behaviour and the AC**, even if minor. The one deviation recorded (AC3's "Product in action" section omission) is an operator-reviewable, explicitly-documented scope decision permitted by the plan itself, not an unrecorded gap — now being actively addressed by a new story, `dsa-s7`, created the same day at the operator's own request.

**Live browser render check — completed same-day, against real staging (`wuce-staging.fly.dev`):** After this DoD was originally written, the operator reconnected the Claude-in-Chrome extension and asked to verify against real deployed staging rather than the local harness. Directly confirmed on real staging: dark mode renders cleanly (hero, golden-trace demo with real live content, both hero cards with a real live instruction hash, auth panel with all 3 real entry points); the theme toggle was clicked live and correctly switched the whole page to light mode (off-white background, dark text, deeper blue accent); the WCAG contrast gap (AC-independent, tracked as an NFR gap below) was visually confirmed as a real, measurably-soft legibility issue in dark mode on the real button, and visibly crisper in light mode, consistent with the earlier automated measurement; the "Product in action" gap was visually confirmed present exactly as documented. A true mobile-viewport visual pass was attempted but not achievable via this session's own browser tooling (window resize does not reach a true narrow width in this environment) — AC5 remains verified via Playwright's more reliable `setViewportSize()` mechanism, not a live visual pass specifically. Full detail: `decisions.md`'s dedicated entry.
...
7. **Superseded, same day**: item 4 above (previously listed as AC3's "Product in action" omission needing a follow-up) is now actively addressed — `dsa-s7` was created the same day at the operator's own explicit request, DoR-signed and ready for implementation. Not yet merged, so `dsa-s3`'s own AC3 deviation remains recorded here until `dsa-s7` ships.
...
3. **The live browser render check was attempted 4 separate times across this story's delivery** (Task 1, Task 2, `/verify-completion`, and this DoD) — the Claude-in-Chrome extension remained disconnected every time. All 4 attempts are documented in `decisions.md`/this DoD with consistent, non-repetitive RISK-ACCEPT reasoning, backed by genuinely strong automated evidence (real computed-style assertions, real layout structure checks, real viewport-overflow measurements) each time. **Update, same day**: the operator reconnected the extension and the live check was completed against real staging — see the amended AC Coverage table and the dedicated "Live browser render check" note above. The root cause of the recurring disconnects was never diagnosed (a manual re-login/re-auth resolved it, consistent with a stale-auth-token or session-boundary theory, but not confirmed). `/improve` candidate unchanged: if this pattern recurs in future sessions, worth a dedicated investigation rather than treating each occurrence as independent.
4. **A real live-staging session, once the tooling was working, surfaced one new limitation worth recording**: the browser-window-resize tool does not reliably reach true narrow mobile viewport widths in this environment (confirmed twice this feature — once during `dsa-s2`'s own delivery, again here) — it plateaus around 980-1000px regardless of the requested width. This does not affect this story's own AC5 verification (Playwright's `page.setViewportSize()` is a separate, working mechanism, already used successfully throughout this feature), but it does mean a true manual/visual mobile check is not currently possible via this session's own browser automation tooling — only the automated-assertion route is reliable for narrow-viewport verification. `/improve` candidate: investigate whether this is a Claude-in-Chrome tool limitation, an OS/window-manager constraint, or a display-scaling artifact, since it blocks one specific class of manual verification this repo's own convention would otherwise call for.
