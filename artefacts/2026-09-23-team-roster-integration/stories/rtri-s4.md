...
- **D37 (injectable adapter):** none introduced. This reuses the existing direct-DB-access convention already established for `identity-links.js`/`team-management.js` (no I/O adapter, per `tir-s1`/`tir-s2`'s own precedent).
