...
## /verify-completion manual walkthrough finding — Critical, NOT YET FIXED (blocks branch-complete)

The plan's own "Post-implementation note for /verify-completion" below required manually opening the live page for Scenario 1 (`phase4`, 205 real documents) before claiming AC1 complete — this caught a severe, previously-undetected bug that every fixture-based automated test (all built from 1-2-artefact synthetic fixtures) missed entirely.

**Finding:** started the local server (`NODE_ENV=test`, seeded a test session via `/test/session`) and fetched `/features/2026-04-19-skills-platform-phase4` for real. `find artefacts/archived/2026-04-19-skills-platform-phase4 -type f | wc -l` confirms 205 real files exist (54 of them under `dor/`, 27 under `stories/`). The rendered page shows only **29** document links total — **176 real documents (86%) are silently missing**, a direct violation of AC1's "none are missing" requirement.

**Root cause:** `renderArtefactMatrix`'s cell-building loop (`src/web-ui/routes/features.js:598`, pre-existing code not introduced by cat-s4) does `byColumn[_deriveMatrixColumn(a.path || '')] = a;` — a plain overwrite, not an accumulation. This was always latently unsafe, but harmless before cat-s4 because the matrix was only ever fed correctly-registered, one-real-story-per-row data, where each story legitimately has at most one `dor.md`, one `plan.md`, etc. — one artefact per (row, column) cell, by construction. cat-s4's routing change (Critical Finding #3 above) is what newly violates that invariant: for a feature like `phase4` where only ~4-5 of ~27 real stories get their own inferred bucket, the other ~22 stories' worth of `dor`/`plan`/`review`/etc. artefacts all land in the SAME shared `Unregistered` catch-all row, where `_deriveMatrixColumn` maps every `dor/`-subdirectory file to just 2 possible keys (`dor`, `dor-contract`) regardless of which underlying story it belongs to — so all but the last of ~52 real `dor/` files silently vanish into that one overwritten cell, with no error, no test failure, and no visible symptom short of manually counting real files against what renders.

**Fix (in scope — `features.js` only, no `artefact-trace.js`/`inferGroups` change needed):** change `byColumn` from a single-value map to an array-accumulating map, and render every artefact in a cell (not just the last one) — e.g. multiple `<a class="doc-matrix__tick">` links space-separated within the same `<td>` when a cell holds more than one real artefact. This must not change the single-artefact-per-cell case's markup at all (AC4's byte-identical golden fixture, and every existing `fadm-s1`/`fapg-s1` fixture, only ever has 1 artefact per cell — confirm this explicitly with a test using each fixture's real data, not just a new synthetic multi-artefact fixture, before considering this fixed).

**Status: ✅ FIXED (e47866a4).** `byColumn` changed from a single-value overwrite to an array-accumulating map; every artefact in a cell now renders (space-separated tick links) instead of only the last one written. A new regression test uses the REAL on-disk `phase4` feature (not a synthetic fixture) — counts real files via a filesystem walk matching `buildArtefactTrace`'s own rule, renders the real trace, and asserts the rendered distinct-link count equals the real file count. Confirmed failing (29 vs 205) against the pre-fix code, passing (205 vs 205) after. Re-verified independently, twice: (1) the automated suite — targeted file 24/24 including the new regression test and the still-unchanged AC4 golden-fixture byte-identical test, `check-fadm-s1-document-matrix.js` 27/27, `check-fapg-s1-group-artefacts-by-story.js` 7/7, full suite 626 files/1 pre-existing-unrelated-failure/0 regressions; (2) manually, by restarting the real local server and re-fetching `/features/2026-04-19-skills-platform-phase4` — **205 distinct `/artefact/` links now render** (was 29), confirmed via `grep -oE 'href="/artefact/[^"]+"' | sort -u | wc -l` against the live HTTP response, not just the test suite. AC1 now genuinely holds for its own named acceptance scenario. `/verify-completion` can proceed.

---

## Mandatory final review (whole-branch diff against all ACs) ✅ Approved (ab3bf2a8)

**Note (found after this review, see the section above):** this review's own AC1 check traced only bucket *membership* (which array an artefact lands in) end-to-end for all 205 `phase4` artefacts, correctly finding 205/205 accounted for there — but did not render the actual HTML and count visible tick marks, which is what the subsequent manual walkthrough did and where the real gap was found. Bucket-membership completeness is necessary but not sufficient for AC1; final-render completeness is the actual bar the AC sets, and no review step before the manual walkthrough checked it.
