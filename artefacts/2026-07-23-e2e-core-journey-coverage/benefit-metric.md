...
**Coverage caveat added 2026-07-23 (a2ccf-s1 corrected finding):** Scenario A's CI-blocking gate automatically covers A1 (staging-safe auth), A3 (product/feature/ideate creation), and A4 (session-resume) only. A2 (Stripe test-mode plan selection/billing activation) contributes zero automated CI-blocking signal — all three of its acceptance criteria (AC1, AC2, AC3) were confirmed, via real CI trace evidence, to be blocked by Stripe's own hCaptcha bot-detection on the checkout-submission step itself, a genuine third-party constraint this repo cannot automate past (see `artefacts/2026-07-23-a2-stripe-ci-checkout-flake/decisions.md`). This is not a scope reduction the team chose casually — worker concurrency and timeout adjustments were both tried and ruled out first, and the same flow passes reliably outside CI (confirmed via repeated local/interactive runs). A2's billing/plan-activation correctness is instead verified via its manual verification script (`artefacts/2026-07-23-e2e-core-journey-coverage/verification-scripts/a2-stripe-test-mode-plan-selection-verification.md`), run by a human before merge or at a documented cadence. Scenario A's "minimum validation signal" (row above) is therefore satisfied by A1 + A3 + A4's automated coverage plus A2's manual coverage, not by 4 fully-automated stories.
