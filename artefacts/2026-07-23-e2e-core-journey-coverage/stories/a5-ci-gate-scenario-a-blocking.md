...
I want to **have Scenario A (A1, A3, A4 automated and CI-blocking; A2's spec file also runs but is manual-verification-only per the a2ccf-s1 corrected finding — see Architecture Constraints) run in CI on every PR and block merge on failure, while the 29 existing local-mocked specs remain non-blocking exactly as today**,
...
- **Clarification added 2026-07-23 (a2ccf-s1 corrected finding):** A2 (`a2-stripe-test-mode-plan-selection`)'s AC1, AC2, AND AC3 tests are ALL now skipped specifically in this job (`process.env.CI === 'true'`) — Stripe's own hCaptcha bot-detection on hosted Checkout blocks the checkout-submission step itself for all three ACs, not merely the post-checkout redirect that only AC2/AC3 depend on (real trace evidence, including AC1's own browser-page network trace, in `artefacts/2026-07-23-a2-stripe-ci-checkout-flake/decisions.md`); all three are reclassified as manual-verification-only, not removed. This **corrects an earlier same-day version of this clarification**, which stated only AC2/AC3 were skipped and that A2's AC1 remained the CI-automated signal — that premise did not hold: AC1 failed in every real CI run observed (4 of 4) with the identical hCaptcha stall signature. **A2 now contributes ZERO automated CI-blocking signal to this gate.** This story's own AC1/AC2 ("the Scenario A E2E job fails/passes") are unaffected in wording — "the job passes" already meant "every test the job actually runs passes" — but the gate's *composition* changes: Scenario A's CI-blocking, automated signal is now **A1 + A3 + A4 only**; A2 (billing/plan-activation) is verified via its manual verification script exclusively. No re-review of this DoR sign-off is triggered by this correction (see the DoR's own post-sign-off clarification).
