...
**AC3:** Given an active `/ideate` session, When the spec drives at least 2 turns of the ideation conversation (with a bounded retry — up to 3 attempts per turn — if the model does not emit a canvas marker on the first attempt), Then, if canvas markers are emitted, the visual canvas DOM contains rendered card/block elements corresponding to them, and the canvas updates (new elements appear) between turn 1 and turn 2 — not a static/frozen canvas. If no marker is emitted after the bounded retries, this AC is exercised via the manual verification scenario in the test plan instead of failing the automated CI run, matching this repo's established handling of model-instruction-compliance ACs (see `workspace/capture-log.md`, 2026-06-16, inc5).
