...
**Follow-up actions:**
1. ~~The verification script's 4 manual click-through scenarios were never executed against a real deployed instance.~~ — partially closed 2026-09-12: a genuine live Chrome click-through was attempted first (6 real historical journeys checked on `wuce-staging.fly.dev`), but none currently exercise the `dsh-s3` chat-split view this fix lives inside — see `cdpl-s1`'s own DoD Observation #5 for that separate, newly-found gap. Verified instead via direct execution of the real, unmodified production handler with real diagram content, confirming via actual script execution (not just text presence) that all 3 functions are real and callable with zero `ReferenceError`. Full method in `cdpl-s1`'s own DoD Observation #4 (shared fix, one verification pass covers both stories' DoDs).
2. ~~`pipeline-state.json` bookkeeping was stale...~~ — done, corrected in this DoD's own original write.

---

## DoD Observations

1. **Backfilled 2026-09-12, following a wider stocktake for web-UI stories lacking real evidence of cohesion.** See `cdpl-s1-dod.md` DoD Observation #4 for the full live-verification attempt and its result (a real, unmodified production-handler run with real diagram content and actual script execution, since no real historical page on this deployment currently reaches the code path in question). Also see Observation #5 there for the separate `dsh-s3` gap this surfaced.
2. **This is a retroactive DoD, written 4 days after merge.** No `/definition-of-done` run occurred immediately after PR #752 merged — the story sat at `branch-complete`/`prStatus: draft` in `pipeline-state.json` despite the PR being merged, the exact "short-track story never reaches DoD" gap this repo's own `CLAUDE.md` documents as a historical pattern (see the Short-track section, referencing `pcr-s1`/`stis-s1`/`tst-s1`/`jlc-s1`/`cfg-s1`). Found while surveying overall pipeline state for other stale bookkeeping, not through any dedicated audit — worth treating as a signal that this class of gap may still be recurring silently for other short-track stories beyond the five already documented.
