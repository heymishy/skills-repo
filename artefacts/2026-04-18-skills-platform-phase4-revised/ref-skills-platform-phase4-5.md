...
*What breaks at scale:* At 5 squads in a dogfood context this limitation is acceptable with documentation. At 50 squads in a regulated enterprise (where RBNZ BS11 or equivalent applies), the second-line framing is a regulatory posture, not an architectural one. An examiner will probe this.
...
- 4.B.3 — Configurable commit format validation: governance package supports operator-configured commit message format (e.g. SPPX-XXXX Jira story reference; other consumers: their own format) as structural precondition before advancing pipeline state. Format rules live in `context.yml`, not hardcoded.
...
- 4.B.6 — Upstream authority model: resolves the P4.4 open question from PR #98 — is `heymishy/skills-repo` the authoritative upstream for direct consumer consumption, or does the productisation fork remain the publishing source? Phase 4 cannot complete Theme B without this decision.
...
- 4.B.9 — Point-in-time audit snapshot at gate close, pushed to consumer-configured platform. The primary use case: six months after delivery, a risk reviewer or external auditor needs to confirm what was built, which governance standards applied, and whether every gate passed — without asking the delivery team for access to their repository. When a gate passes (DoR sign-off, DoD verification, assurance gate), the platform produces a self-contained snapshot of the audit chain at that moment — skill hashes, artefact hashes, gate verdicts, trace entries — and pushes it to a consumer-configured external platform (e.g. Confluence page, SharePoint, compliance archive). The snapshot gives the auditor that answer on a platform they already control. This is distinct from distribution sub-problem 1d (interaction-surface exclusion), which concerns disciplines who cannot *contribute* to governance through the current VS Code + git toolchain. 4.B.9 addresses a related but different group: people who need to *read and verify* governance evidence but have no ongoing relationship with the delivery repository. Both groups are excluded by the same underlying cause (git-native evidence store), but the solution shapes differ — Theme C removes the contribution barrier; 4.B.9 removes the evidence-access barrier. Forward reference to Theme F (second-line independence) and Spike C sub-question C5 (push mechanism selection).
...
The audit snapshot mechanism (Theme B deliverable 4.B.9) strengthens the second-line independence claim structurally. When gate evidence is pushed to an external platform at gate close, a RBNZ examiner can access the audit trail — skill hashes, gate verdicts, artefact chain — without repository access and without depending on the platform team to provide it. The evidence lives outside the platform team's git repo, on a platform the risk function controls or has independent access to. This is the difference between "we can show you the audit trail" (platform team mediates access) and "the audit trail is already on your platform" (risk function has independent access). The reason this distinction matters: if the team being assessed controls where the audit evidence lives, an examiner cannot independently verify that the evidence is complete — that no gate failures were omitted, no trace entries removed, no skill hashes altered after the fact. When the evidence store is outside the delivery team's control, that assurance becomes structural rather than a claim the team makes about itself. Organisational independence of the evidence store is a precondition for credible second-line independence claims under BS11.
...
**C5 — Audit snapshot push mechanism (connects Theme B deliverable 4.B.9 to sub-problem 1d).** When a gate closes and the platform produces a point-in-time audit snapshot, what pushes it to the consumer-configured external platform? The leading hypothesis is **(b) CI pipeline post-gate-pass step**: the assurance gate already runs in CI (the existing GitHub Actions workflow), so extending a passing gate to push the snapshot to a configured external platform is a natural extension of existing architecture. This avoids giving the CLI network access at invocation time (preserving C11 purity — the CLI works offline; CI handles the outbound push) and keeps `trace-report.js` as a purely read-only reporting tool rather than giving it external side effects. Craigfo's PR #155 CLI approach is not disrupted by this model — the CLI produces the evidence; CI pushes it. Two alternatives the spike should test against this hypothesis: (a) CLI command (`emit-trace` or equivalent in craigfo's approach) — operator or CI invokes explicitly after gate pass, strongest C11 compliance but requires operator action or CI step configuration and gives the CLI a network-dependent code path; (c) `trace-report.js` directly via `--push` flag — extends the existing audit report tool with a push capability using a platform adapter pattern analogous to `context.yml`'s `vcs.type` adapter, but conflates reporting with side effects and makes the tool harder to reason about. The spike should evaluate which mechanism — or which combination per interaction surface — delivers on the design intent without creating a persistent hosted service (C11) or requiring the platform team to mediate access (Theme F independence).
...
*Output:* Distribution architecture with a specific design response per sub-problem, tradeoffs named, migration path from Phase 3 installs. Must address: update propagation to non-git consumers, audit chain integrity when skills are fetched rather than vendored, conservative pinning without channel severance, consumer enterprise network and security constraints, auditor discoverability of governance content in a sidecar install.
...
**D2 — Existing tool landscape.** Where do product managers, business leads, business analysts, and risk reviewers at the consumer organisation actually work day-to-day? Map per discipline before assuming a specific channel. The channel that requires the least behaviour change is the one that gets adopted.
...
**Audit snapshot constraint alignment (deliverable 4.B.9).** Three constraints directly shape the audit snapshot design: C1 (audit evidence survives repo deletion — the snapshot on an external platform is evidence that exists independently of the git repository; if the repo is deleted, the gate verdicts and skill hashes are still accessible); C4 (gate verdict durably recorded — the snapshot captures the gate verdict at the moment it was made, not a later reconstruction; the human approval is recorded as part of the snapshot); C11 (no persistent hosted service — the push mechanism is a CLI step or CI pipeline action invoked at gate close, not a running service that monitors state changes). The snapshot does not replace the git-native trace chain; it produces a second copy of the evidence on a platform the auditor controls.
