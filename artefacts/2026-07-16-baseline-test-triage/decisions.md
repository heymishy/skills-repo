...
---
**2026-07-16 | RISK-ACCEPT | subagent-execution (tst-s1) — CORRECTION: check-md-3-adr.js reclassified from Fixed to Deferred**
**Decision:** After initially classifying `tests/check-md-3-adr.js` as category (a) Fixed (its T4 logic was genuinely corrected and verified 9/9 passing standalone with no time limit), a subsequent full `node scripts/run-all-tests.js` verification run showed it still appears in the failed-files list. Reclassified to category (b)/(c) Deferred and kept in `tests/known-baseline-failures.json` rather than silently leaving the earlier "Fixed" claim unverified against the real signal.
**Alternatives considered:** (1) Leave it classified as Fixed since the standalone logic fix is real and correct (rejected — `scripts/ci-test-regression-check.js` gates on a full-suite log, not a standalone run; leaving it out of the baseline while it structurally can never pass inside the aggregate suite would make every future PR's CI run falsely flag it as a new regression, forever). (2) Increase `run-all-tests.js`'s per-file timeout so T4's nested `npm test` can complete (rejected — explicitly out of scope; `scripts/run-all-tests.js`'s own discovery/timeout mechanism is pcr-s1's scope, not this story's, per the DoR contract's MUST NOT touch list).
**Rationale:** Confirmed directly: `run-all-tests.js`'s `spawnSync` call applies a hard 120-second timeout per child process (`timeout: 120000` in `scripts/run-all-tests.js`). `check-md-3-adr.js`'s T4 check, when its `npm_lifecycle_event` guard doesn't fire (i.e. whenever invoked directly via `node scripts/run-all-tests.js`, not through `npm test`), runs a full nested `npm test` that takes several minutes — always exceeding the 120s budget, so the child process is killed (not failed on a real assertion) every time. The fresh full-suite log confirms this: T4's header prints, then the log jumps straight to the next test file with no completed Results line for `check-md-3-adr.js`. The T4 logic fix itself (repointing at `scripts/ci-test-regression-check.js`'s real baseline instead of a stale hardcoded pattern list) is still correct and kept in the code — it is a genuine improvement for anyone who runs this file standalone with adequate time — but it cannot resolve the file's status inside the aggregate suite's own timeout constraint. This is exactly the kind of premature "Fixed" claim this story's own standard (verify against the real full-suite signal, not just a standalone run) exists to catch — caught here before being reported as final rather than after.
**Made by:** Claude (agent), tst-s1 inner loop, 2026-07-16
**Revisit trigger:** File a follow-up to either extend `run-all-tests.js`'s per-file timeout for known-slow files (a pcr-s1-scope change) or redesign `check-md-3-adr.js`'s T4 check to not require a multi-minute recursive `npm test` call at all.
---
...
---
**2026-07-16 | RISK-ACCEPT | post-merge fix (real CI run, PR #484) — 5 files restored to baseline after local-vs-CI discrepancy**
**Decision:** `tests/run-gpa-tests.js`, `tests/check-bri-s3.5-nfr-stripe-keys.js`, `tests/check-gpa-sc06-source-path-guard.js`, `tests/check-lab-s3.2-stripe-checkout.js`, and `tests/check-lab-s3.4-stripe-webhook.js` were removed from `tests/known-baseline-failures.json` during this story on the basis of local test runs (both this agent's own and an earlier independent operator-run sweep) showing all 5 passing. The actual GitHub Actions CI run on this PR showed all 5 still failing, correctly triggering `scripts/ci-test-regression-check.js`'s gate as "new regressions not in the known baseline." Restored all 5 to the baseline rather than investigating each one's local-vs-CI discrepancy under time pressure.
**Alternatives considered:** Investigate why each of these 5 passes locally but fails in CI (rejected for immediate action — likely environment-dependent, e.g. a Stripe test-mode env var or a source-path assumption that differs between a Windows local dev checkout and the Linux GitHub Actions runner, per a quick read of `check-bri-s3.5-nfr-stripe-keys.js`'s and `check-gpa-sc06-source-path-guard.js`'s own env/path-sensitive logic — but confirming the exact cause for all 5 needs its own dedicated investigation, not a rushed fix to unblock this PR).
**Rationale:** CI (GitHub Actions, Linux) is the authoritative environment for this repo's regression gate, not a local development machine. "Passes locally" is necessary but not sufficient evidence to remove a file from the baseline — this is the same class of environment-assumption error already documented elsewhere in this repo (e.g. `bri-s2.5`'s decisions.md, "verify-completion passed locally is not equivalent to this CI pipeline works"). Restoring these entries keeps the regression gate accurate and green without asserting anything false.
**Made by:** Claude (agent), post-merge CI investigation, 2026-07-16
**Revisit trigger:** File a follow-up to investigate why these 5 files behave differently in CI vs. local (likely a missing/different env var or path assumption) before attempting to remove them from the baseline again.
---
