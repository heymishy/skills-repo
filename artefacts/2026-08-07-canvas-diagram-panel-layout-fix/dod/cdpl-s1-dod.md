...
6. **Precondition now fixed (2026-09-12).** Observation #5's root cause is closed by `ntpg-s1` (`artefacts/2026-09-12-nonstream-turn-persist-gap`, PR #865, merged): the non-streaming JSON API turn-completion path was silently never writing `session_turns`. A real live Chrome click-through of `sw-canvas-fs-btn` is now genuinely reachable for any *newly* completed `/design`/`/definition` stage with a diagram (not for the 6 already-completed journeys sampled above, which predate the fix and remain on the plain fallback per `ntpg-s1`'s own declared out-of-scope). Not executed as part of this pass — it requires driving a real `/design` session to a diagram-producing completion, a larger lift than this backfill — but the blocker that made Observation #4's `vm.runInContext` fallback necessary no longer exists for future verification.
