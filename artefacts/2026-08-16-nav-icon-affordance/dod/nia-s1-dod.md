...
**Live Chrome verification executed 2026-09-11 (backfilled — see DoD Observation #4), closing Follow-up action #1:** Performed on `wuce-staging.fly.dev`, applying the `getComputedStyle` rigor this repo's own `sob-s4` story established (a `className`/markup check alone proves the string is well-formed, not that it resolves to a visible effect):

- **AC1/AC2 (sign-out):** `.sw-signout` confirmed genuinely visible (`display: flex`, `visibility: visible`, `opacity: 1`, real `color`/`font-size` — not `display:none`/`opacity:0`-hidden), text content `"↗Sign out"`, `href="/auth/logout"` unchanged, `onclick="return confirm('Sign out of wuce?')"` present exactly as specified. Not actually clicked (per this session's own standing rule against triggering real browser dialogs) — confirmed via DOM/attribute inspection instead of a real confirm-dialog round-trip.
- **AC3/AC4 (theme toggle) — found a real, non-regressive surface change:** `.sw-theme-toggle` is **no longer present on `/dashboard`or any shared-shell page**. Reading `html-shell.js` directly explains why: a later story, `si-s1`, moved `renderThemeToggle()`'s call site out of the shared shell into Settings' own Profile tab (`routes/settings.js`'s `renderProfileTab()`) — a deliberate relocation, not a defect, and not something `nia-s1`'s own scope could have anticipated (`si-s1` postdates it). Located and verified on `/settings` instead: `getComputedStyle` on both icon spans confirms real, resolved rendering — in `data-theme="dark"`, `.sw-theme-toggle-icon--dark` (☾) is `display:block` with `font-size:13px`/`color:rgb(128,128,128)`/`opacity:1` (genuinely visible, not just present in markup), `.sw-theme-toggle-icon--light` (☀) is `display:none`. Called `window.swToggleTheme()` directly (not a real click, same dialog-avoidance reasoning) and confirmed `data-theme` flips `dark→light`, `localStorage['sw-theme']` persists the new value, and the icon `display` states swap correctly — then flipped back to leave the page in its original state. No `◑` glyph found anywhere. AC3/AC4 both hold, on their current (relocated) surface.
- **Honest limitation:** this repo's browser-automation tooling could not produce an actual screenshot in this environment (`Cannot take screenshot with 0 width` — the same limitation noted in `sob-s4`'s own DoD). The "reads as unambiguous to a real human eye" question the original RISK-ACCEPT named is therefore still verified via `getComputedStyle` + semantic-glyph inspection (real Unicode ☀/☾ characters, real opacity/color/size), not via an actual rendered-pixel visual check. This is materially stronger evidence than the markup-only check the original RISK-ACCEPT was written against, but it is not identical to a human looking at the rendered page. Follow-up action #2 (below) remains open for exactly this reason.
...
| B2 (CSS-layout-dependent AC classification) | ✅ (RISK-ACCEPT, closed 2026-09-11) | AC3's "does the icon look right to a human" dimension classified as RISK-ACCEPT + manual smoke test per CLAUDE.md's B2 rule, logged in `decisions.md`, with a corresponding Scenario 3 in the verification script. Executed 2026-09-11 via live Chrome `getComputedStyle` verification on `wuce-staging.fly.dev` (see Test Plan Coverage above) — real screenshot-based visual inspection was not possible in this automation environment, so this closes the check at the same fidelity as `sob-s4`'s own precedent (computed-style resolution, not a pixel-level human look), not a full replacement for an actual human glance. |
...
**Follow-up actions:**
1. ~~Execute the verification script's Scenario 3 (manual visual check of the sun/moon icon against real staging)~~ — done, 2026-09-11 (see Test Plan Coverage and NFR Status above). Closed with an honest caveat: computed-style verification, not an actual screenshot-based human look (this environment's browser tooling cannot capture screenshots).
2. [Owner: Hamish King, still open] Confirm with the beta user (or a fresh live check on a real mobile Safari device) that the sign-out control and theme toggle now read as intended on mobile Safari specifically, matching how signals #3/#4 were originally reported — this session's own verification was desktop-viewport browser automation, not a real mobile-device check.
3. **New, found during this backfill:** the theme toggle's real location has moved (`si-s1`, shared shell → Settings' Profile tab) since this story shipped. Not a defect in this story, but worth a note for anyone reading `nia-s1`'s own story text cold in the future — it describes "top-right of the nav, every page," which is no longer accurate; the fix itself is unaffected and still correct at its current location.
...
4. **Backfilled 2026-09-11, following a repo-wide DoD-verification-method stocktake:** this story's own RISK-ACCEPTed manual smoke test (Follow-up action #1) had sat unexecuted for nearly a month before this backfill closed it — the longest-open example found in that stocktake's triage list. Closing it surfaced a genuinely useful finding a purely-automated re-run of the original test suite never could have: the element being checked had quietly relocated to a different page entirely (`si-s1`), months after this story's own tests were last green. The automated tests still pass today because they test the *function* (`renderThemeToggle()`'s own output), not *which page calls it* — exactly the kind of gap live verification exists to catch that a passing test suite alone cannot.
