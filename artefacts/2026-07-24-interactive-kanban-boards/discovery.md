...
~~[ASSUMPTION] The existing `skills gate-advance`/`advance` machinery can be safely invoked from a web request handler (not just the CLI) without any behavioural difference in validation, hash-checking, or state-write safety~~ — **RESOLVED via /clarify, 2026-07-24:** confirmed by direct code inspection. `src/enforcement/cli-advance.js`'s `advance()` and `cli-gate-advance.js`'s `gateAdvance()` are already plain, pure, `require()`-able functions — no `process.exit()`, no subprocess/`child_process` calls (explicitly documented as satisfying ADR-H7.1, "no child_process spawning"). They return `{ exitCode, stdout, stderr }` rather than writing to the real stdout or exiting the process. `bin/skills`' own CLI is just a thin wrapper calling these same functions. This means a web route handler can `require()` and call `gateAdvance()` directly — identical validation, identical hash-checking, identical state-write path as the CLI, with zero behavioural difference. No new integration risk here; this is a low-complexity, already-safe integration point.
