...
- **Client-side DOM patch, not full reload, for a POST action on a page with live in-progress state.** A page with an open SSE conversation or unsaved draft loses that state on `window.location.reload()`; patch the specific DOM element(s) from the POST response instead, following the existing `attachCardHandlers` precedent. Reserve reload for static/administrative pages. See `.github/standards/web-ui/web-ui-patterns.md`.
- **A visible-marker/flag feature that can be set on a not-yet-reached downstream item needs an explicit resolution-path decision, not an implicit one.** When a feature marks item N as "affected by a change to item M" (M < N in some sequence) and the marker's own stated purpose is "so it doesn't persist forever with no resolution path," decide explicitly at DoR time whether marking is scoped to already-reached items only, or extends to not-yet-reached ones — and if the latter, name the mechanism that resolves a mark on an item reached for the first time (not just one that's reopened). Confirmed via res-s4 (`2026-08-27-revise-earlier-stage`): flags could land on downstream stages the operator hadn't reached yet, and only the REOPEN path cleared a flag — a stage reached for the first time while flagged had no clearing mechanism at all. RISK-ACCEPTed as a documented scope boundary (decisions.md, 2026-08-29, finding O2) rather than silently shipped, but the DoR contract stage would have been the cheaper place to make this an explicit choice.
...
| Searching only the files a story's own investigation already has open to inventory a shared UI component's render sites | A file-scoped search can correctly widen a DoR contract's touch-point count and still under-count, if a structurally identical render site exists in a file the investigation never opened. Evidenced res-s4 (2026-08-29): a DoR contract correction found 2 render sites in `journey.js`, missing a 3rd, identical one in `skills.js`, found only at final review. | `grep -rn` the component's own class/marker name across the whole repo before finalizing a DoR contract's touch points — not just the files the story's ACs already mention. See `.github/standards/web-ui/web-ui-patterns.md`. |
...
| Classifying an AC's marker/element as NOT CSS-layout-dependent solely because its presence is DOM/markup-verifiable | "The marker element exists in the DOM" and "the marker is visually reachable without extra interaction (scrolling, clicking an overflow control) at a normal viewport" are different claims — a DoR gate that only checks the first can pass an AC that fails a real user's actual experience of it. Evidenced 2026-08-29: res-s4's DoR (`H-E2E` gate) classified its flag-marker AC as not CSS-layout-dependent on exactly this reasoning; a live browser check (run only after DoD, on operator request) found the marker was genuinely present in the DOM on all three render sites but NOT visible without horizontal scroll on a normal-width viewport on one of them — a real, if minor, discoverability gap no automated test caught. A `/definition-of-ready` process proposal for this exact gate-classification question is deferred behind an existing pending proposal on that skill (`workspace/proposals/2026-08-14-definition-of-ready-improve-proposal.md`) — this row preserves the finding until that's resolved. | When classifying an AC per CLAUDE.md's B2 rule, ask whether the marker/element renders inside a container that can overflow, truncate, or otherwise require interaction to become visible — not just whether the marker's OWN presence is DOM-verifiable. If genuinely uncertain, treat as CSS-layout-dependent (RISK-ACCEPT + manual smoke test, per B2) rather than defaulting to "not applicable." |
| Treating "the fix's own new tests pass" as proof the fix satisfies the realistic user-action sequence that motivated it | A fix that adds UI feedback for a specific interaction (click X, expect to see Y) can have its own tests pass by rendering the page FRESH, while the actual interaction (click, then look at the SAME already-loaded page) never triggers that render logic at all. These are different claims and a green test on the first does not establish the second. Evidenced 2026-08-29, res-s4: Task 5's fix (a flag marker's render logic) was correct and its own tests passed — a second round of the mandatory final cross-task review found the marker never actually appeared after the click that was supposed to produce it, because nothing re-ran the render on the already-loaded page. A `/subagent-execution` process proposal to make this an explicit final-review trace step is deferred behind an existing pending proposal on that skill (`workspace/proposals/2026-08-26-subagent-execution-precedent-verification-improve-proposal.md`) — this row preserves the finding until that's resolved. | When a fix adds UI feedback for a specific user action, trace the actual action sequence by hand (click → expect visible result on THIS page, not a fresh reload) as part of that fix's own review — not just a fresh-page-load test. |
