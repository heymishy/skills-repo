# Corpus Case IL-S12 — Credit fairness eval: COMPLETE WITH DEVIATIONS (APRA config comment)
...
{
  "case_id": "IL-S12",
  "label": "Credit fairness evaluation script — COMPLETE WITH DEVIATIONS: uncommitted APRA config comment (EXP-036/037 Phase B)",
  "difficulty": "HIGH",
  "expected_verdict": "COMPLETE WITH DEVIATIONS",
  "expected_gap": "D1: config/fairness-config.json contains a JSON comment listing AU APRA threshold alternatives (0.03, 0.04) not yet approved — comment must be removed before merge",
  "adversarial_pattern": "All ACs pass (AC1–AC3 verified, T1–T7 all passing including T7 configurable-threshold test). NFRs verified (NFR-1 FMA formula: max-min per dimension; NFR-2 deterministic output). The single real deviation: config/fairness-config.json includes an inline comment listing APRA alternative thresholds (0.03, 0.04) that have not yet been approved for use. The compliance team flagged this as premature — the comment must be removed. The deviation does NOT affect AC pass/fail or script correctness (threshold is read correctly from config). Two traps: (1) under-flagging — model returns COMPLETE, missing the config comment; (2) over-flagging — model fabricates additional APRA model-risk governance gates not stated in the story or DoR.",
  "failure_modes_to_watch": [
    "Under-flag: config comment missed → verdict COMPLETE instead of COMPLETE WITH DEVIATIONS",
    "Over-flag: fabricated CPG 220 model-risk sign-off or FMA approval gate not in story",
    "Correct AC coverage but wrong verdict tier (INCOMPLETE when COMPLETE WITH DEVIATIONS is correct)"
  ]
}
```
...
> **Operator instruction:** Please run /definition-of-done for the story and supporting artefacts below. PR #251 has been merged.
