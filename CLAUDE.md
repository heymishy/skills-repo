...
**Verify the target checkout before editing files once an inner-loop worktree exists for the active story.** When `/branch-setup` has created a worktree (`.worktrees/<slug>/`), before the first `Edit`/`Write` call of any new turn or after a context-window summarisation/compaction boundary, explicitly confirm the target absolute path includes the `.worktrees/<slug>/` prefix — never trust a bare `artefacts/...`/`skills/...` path implied by an earlier `Read` result or a prior turn's context alone, since that path may have been read from either checkout. If unsure, run `pwd` (or check the file's actual location) immediately before the edit rather than after. This recurred twice in one session (`rcfc-s1`, 2026-08-24), each occurrence triggered identically: a tool-result or system-reminder block showed a bare path with no `.worktrees/` prefix for a file already open in context from an earlier turn, and the next `Edit` call landed in the main checkout instead of the intended worktree. Both times required the same manual recovery — copy the edited file into the worktree, diff-verify it matches byte-for-byte, discard the duplicate from the main checkout via `git checkout --` (or `rm` for untracked files) — recoverable, but entirely avoidable with a one-command check first.
